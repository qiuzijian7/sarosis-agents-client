...
(async () => {
	const s = [
		'<scr ipt>', '<sc ript>', '<script >', '<scr\u200Bipt>',
		'alert(1)', 'javascript :', 'java script:', 'javas cript:', 'java.script:',
		'javascript', 'script:', 'javascript:void',
		'<script', 'script>',
	];
	for (const x of s) {
		console.log(JSON.stringify(x).padEnd(18), '->', await send('测试 ' + x));
	}
})();