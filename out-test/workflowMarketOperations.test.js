...
class MockWorkspaceContextService {
    _workspaceId;
    constructor(_workspaceId = 'test-workspace') {
 ...
 */
function isVersionHigher(server, local) {
    if (!server) {
        return false;
    }
    if (!local) {
...
suite('WorkflowMarketOperations', () => {
    ...
    setup(() => {
        ...
        installer = new WorkflowInstaller(fileService, logService, workflowStorage, pathService, workspaceService);
    });
    ...
    test('install: 工作流已存在且未指定 force 时抛出冲突错误', async () => {
        ...
        await assert.rejects(() => installer.install(manifest, extractedDir), (err) => err.message.includes('已存在'));
    });
    ...
    test('preparePack: 工作流不存在时抛出错误', async () => {
        await assert.rejects(() => installer.preparePack('non-existent-id'), (err) => err.message.includes('不存在'));
    });
