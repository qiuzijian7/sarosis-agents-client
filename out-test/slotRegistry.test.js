...
suite('SlotRegistry (Phase 1)', () => {
    ...
    function createSlotRegistry() {
        ...
        const kanbanProviders = [];
        const sortByPriority = (arr) => {
            arr.sort((a, b) => b.priority - a.priority);
        };
        return {
            // Memory
            registerMemoryProvider: (provider, priority = 0) => {
            ...
            // Tool
            registerToolProvider: (provider, priority = 0) => {
            ...
            getActiveToolProvider: () => {
                return toolProviders.length > 0 ? toolProviders[0].provider : undefined;
            },
            // Planning
            registerPlanningProvider: (provider, priority = 0) => {
                ...
                return {
                    dispose: () => {
                        ...
                        if (idx !== -1) {
                            planningProviders.splice(idx, 1);
                        }
                    },
                };
            },
            getActivePlanningProvider: () => {
                return planningProviders.length > 0 ? planningProviders[0].provider : undefined;
            },
            // Execution
            registerExecutionProvider: (provider, priority = 0) => {
                ...
                return {
                    dispose: () => {
                        ...
                        if (idx !== -1) {
                            executionProviders.splice(idx, 1);
                        }
                    },
                };
            },
            ...
            // Retrieval
            registerRetrievalProvider: (provider, priority = 0) => {
                ...
                return {
                    dispose: () => {
                        ...
                        if (idx !== -1) {
                            retrievalProviders.splice(idx, 1);
                        }
                    },
                };
            },
            getActiveRetrievalProvider: () => {
                return retrievalProviders.length > 0 ? retrievalProviders[0].provider : undefined;
            },
            // Kanban
            registerKanbanProvider: (provider, priority = 0) => {
                ...
                return {
                    dispose: () => {
                        ...
                        if (idx !== -1) {
                            kanbanProviders.splice(idx, 1);
                        }
                    },
                };
            },
            getActiveKanbanProvider: () => {
    ...
    test('registerMemoryProvider - single provider', () => {
        ...
        const provider = { id: 'mem-1', name: 'Memory 1', loadContext: async () => ({ shortTermMemories: [], longTermMemories: [] }), writeMemory: async () => { }, searchMemory: async () => [] };
    ...
    test('registerMemoryProvider - higher priority wins', () => {
        const registry = createSlotRegistry();
        const low = { id: 'mem-low', name: 'Low', loadContext: async () => ({ shortTermMemories: [], longTermMemories: [] }), writeMemory: async () => { }, searchMemory: async () => [] };
        const high = { id: 'mem-high', name: 'High', loadContext: async () => ({ shortTermMemories: [], longTermMemories: [] }), writeMemory: async () => { }, searchMemory: async () => [] };
    ...
    test('registerMemoryProvider - default priority is 0', () => {
        const registry = createSlotRegistry();
        const first = { id: 'mem-first', name: 'First', loadContext: async () => ({ shortTermMemories: [], longTermMemories: [] }), writeMemory: async () => { }, searchMemory: async () => [] };
        const second = { id: 'mem-second', name: 'Second', loadContext: async () => ({ shortTermMemories: [], longTermMemories: [] }), writeMemory: async () => { }, searchMemory: async () => [] };
    ...
    test('registerExecutionProvider - single provider', () => {
        ...
        const provider = {
            ...
            runAgentLoop: async function* () { yield { type: 'done' }; },
        };
    ...
    test('registerExecutionProvider - higher priority wins', () => {
        const registry = createSlotRegistry();
        const low = { id: 'exec-low', name: 'Low', runAgentLoop: async function* () { } };
        const high = { id: 'exec-high', name: 'High', runAgentLoop: async function* () { } };
    ...
    test('registerKanbanProvider - single provider', () => {
        ...
        const provider = {
            ...
            onDidChangeCards: { event: () => ({ dispose: () => { } }) },
            onDidChangeBoard: { event: () => ({ dispose: () => { } }) },
        };
