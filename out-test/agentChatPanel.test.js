...
suite('AgentChatPanel — Types & Data Structures', () => {
    ...
    suite('IWorktreeItem', () => {
        test('branch worktree', () => {
            const wt = { path: '/repo/worktrees/feat-x', branch: 'feat-x' };
            assert.strictEqual(wt.path, '/repo/worktrees/feat-x');
            assert.strictEqual(wt.branch, 'feat-x');
        });
        test('detached worktree', () => {
            const wt = { path: '/repo/worktrees/detached-1', branch: '(detached @ abc1234)' };
    ...
    suite('IWorkspaceItem', () => {
        ...
        test('multiple workspaces with unique IDs', () => {
            const a = { id: 'a', name: 'A', path: '/a' };
            const b = { id: 'b', name: 'B', path: '/b' };
    ...
    suite('Workspace/Worktree Switching — downstream impact', () => {
         ...
         */
        // ── test helpers ──────────────────────────────────────────────
        const wsA = { id: 'repo-a', name: 'Repo A', path: '/repos/repo-a' };
        const wsB = { id: 'repo-b', name: 'Repo B', path: '/repos/repo-b' };
        const wtMain = { path: '/repos/repo-a', branch: 'main' };
        const wtFeat = { path: '/repos/repo-a/worktrees/feat-x', branch: 'feat-x' };
        /** 创建模拟的工作区列表加载器（每次返回固定列表） */
        function makeWorkspaceLoader(workspaces) {
            let callCount = 0;
            const fn = async () => {
                callCount++;
                return workspaces;
        ...
        function makeWorktreeLoader(mapping) {
            let callCount = 0;
            const fn = async (workspaceId) => {
                callCount++;
        ...
        test('select workspace → callback fires with correct id and name', async () => {
            let capturedId = '';
            let capturedName = '';
            const onSelect = (id, name) => { capturedId = id; capturedName = name; };
            // 模拟用户点击选择 repo-b
        ...
        test('select workspace → worktrees are reloaded for new workspace', async () => {
            ...
            worktreeMap.set('repo-a', [wtMain, wtFeat]);
            worktreeMap.set('repo-b', [{ path: '/repos/repo-b', branch: 'develop' }]);
            let selectedId = '';
            let loadedWorktrees = [];
            const onSelect = async (id, _name) => {
                selectedId = id;
        ...
        test('select worktree → callback fires with correct path and branch', () => {
            let captured = null;
            const onSelect = (wt) => { captured = wt; };
            onSelect({ path: '/repos/repo-a/worktrees/feat-x', branch: 'feat-x' });
            assert.ok(captured);
            assert.strictEqual(captured.path, '/repos/repo-a/worktrees/feat-x');
            assert.strictEqual(captured.branch, 'feat-x');
        });
        ...
        test('worktree loader not called when loading workspaces fails', async () => {
            ...
            const loadWorkspaces = () => Promise.reject(new Error('network error'));
            const loadWorktrees = async () => { worktreeCallCount++; return []; };
        ...
        test('worktree dropdown shows branch name when selected', () => {
            const wt = { path: '/repo/worktrees/my-branch', branch: 'my-branch' };
        ...
        test('IWorkspaceItem readonly — cannot mutate after creation', () => {
            const ws = { id: 'readonly', name: 'Test', path: '/path' };
            // 验证可以读取
