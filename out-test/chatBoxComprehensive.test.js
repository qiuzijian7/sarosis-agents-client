...
function makeContextUsage(overrides = {}) {
    return {
        used: 5000,
        limit: 10000,
        ratio: 0.5,
...
function makeCheckpointInfo(overrides = {}) {
    return {
        id: 'cp-1',
        label: 'Checkpoint 1',
        timestamp: Date.now(),
...
suite('ChatBox Comprehensive - Unit Tests', () => {
    ...
    suite('Chat Mode', () => {
        test('craft mode', () => {
            assert.strictEqual('craft', 'craft');
        });
        test('ask mode', () => {
            assert.strictEqual('ask', 'ask');
        });
        test('plan mode', () => {
    ...
    suite('Stream Phase', () => {
        ...
        test('llm_streaming phase', () => {
            assert.strictEqual('llm_streaming', 'llm_streaming');
        });
        test('tool_executing phase', () => {
            assert.strictEqual('tool_executing', 'tool_executing');
        });
        test('awaiting_approval phase', () => {
            assert.strictEqual('awaiting_approval', 'awaiting_approval');
        });
    });
    ...
    suite('Context Usage', () => {
        ...
        test('empty usage', () => {
            const usage = { used: 0, limit: 10000, ratio: 0, percent: 0 };
            assert.strictEqual(usage.percent, 0);
        });
        test('full usage', () => {
            const usage = { used: 10000, limit: 10000, ratio: 1, percent: 100 };
...
suite('ChatBox Comprehensive - Functional Tests', () => {
    ...
    suite('Agent Management', () => {
        ...
        test('agent with different types', () => {
            ...
            types.forEach(t => {
                const agent = makeAgent({ agentType: t });
    ...
    suite('Integration Scenario - Chat Flow', () => {
        test('complete chat flow', () => {
            ...
            const updatedToolCall = { ...asstMsg.toolCalls[0], status: 'success' };
...
suite('ChatBox Comprehensive - Smoke Tests', () => {
    ...
    test('minimal tool call', () => {
        const tc = { id: '1', name: 't', status: 'success' };
    ...
    test('minimal agent', () => {
        const agent = { id: '1', name: 'A', status: AgentStatus.Idle };
        assert.strictEqual(agent.status, AgentStatus.Idle);
    });
    test('minimal worktree', () => {
        const wt = { path: '/wt', branch: 'main' };
    ...
    test('all chat modes', () => {
        const modes = ['craft', 'ask', 'plan'];
        modes.forEach(m => assert.ok(['craft', 'ask', 'plan'].includes(m)));
    });
    test('all stream phases', () => {
        const phases = ['idle', 'llm_streaming', 'tool_executing', 'awaiting_approval', 'compressing', 'error'];
