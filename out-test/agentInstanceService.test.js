...
suite('Agent Instance Service (Phase 4)', () => {
    ...
    class MockAgentInstanceService {
        ...
        async getInstances(workspaceId) {
            if (workspaceId) {
        ...
        async createInstanceFromTemplate(templateId, workspaceId) {
            const newInstance = {
    ...
    test('updateInstance throws for non-existent', async () => {
        const service = new MockAgentInstanceService();
        await assert.rejects(service.updateInstance('non-existent', { name: 'test' }), /Instance non-existent not found/);
    });
