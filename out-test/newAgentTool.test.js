...
class ThrowingMockStudioService {
    async createAgent(_data) {
...
suite('new_agent Tool', () => {
    ...
    test('T1.5: id contains no random suffix (clean slug only)', async () => {
        const mock = new MockAgentStudioService();
        const result = await handleNewAgentTool({ name: 'Security Auditor', role: 'Auditor', description: 'Audits security' }, mock);
        const parsed = parseResult(result);
    ...
    test('T1.6: fails when name slugifies to empty string', async () => {
        const mock = new MockAgentStudioService();
        const result = await handleNewAgentTool({ name: '!@#$%', role: 'Tester', description: 'Test' }, mock);
        const parsed = parseResult(result);
    ...
    test('T2: fails when name is missing', async () => {
        const mock = new MockAgentStudioService();
        const result = await handleNewAgentTool({ role: 'Developer', description: 'Writes code' }, mock);
        const parsed = parseResult(result);
        assert.strictEqual(parsed.success, false);
    ...
    test('T3: fails when role is missing', async () => {
        const mock = new MockAgentStudioService();
        const result = await handleNewAgentTool({ name: 'Coder', description: 'Writes code' }, mock);
        const parsed = parseResult(result);
        assert.strictEqual(parsed.success, false);
    ...
    test('T4: fails when description is missing', async () => {
        const mock = new MockAgentStudioService();
        const result = await handleNewAgentTool({ name: 'Coder', role: 'Developer' }, mock);
        const parsed = parseResult(result);
        assert.strictEqual(parsed.success, false);
    ...
    test('T5: uses user-provided systemPrompt instead of default', async () => {
        const mock = new MockAgentStudioService();
        await handleNewAgentTool({ ...VALID_RAW_INPUT, systemPrompt: 'You are a meticulous reviewer.' }, mock);
    ...
    test('T5.5: auto-generates default systemPrompt from role + description', async () => {
        ...
        assert.strictEqual(mock.createAgentCalls[0].systemPrompt, 'You are a Reviewer. Reviews code for bugs and best practices');
    });
    // ── T6: 带 model ──────────────────────────────────────────────────────────
    test('T6: passes model to createAgent', async () => {
        const mock = new MockAgentStudioService();
        await handleNewAgentTool({ ...VALID_RAW_INPUT, model: 'gpt-4o' }, mock);
    ...
    test('T9: sets agentType to Planner when specified', async () => {
        const mock = new MockAgentStudioService();
        const result = await handleNewAgentTool({ ...VALID_RAW_INPUT, agentType: 'planner' }, mock);
        const parsed = parseResult(result);
    ...
    test('T12: returns valid IToolResultContent format including systemPrompt', async () => {
        ...
        assert.strictEqual(parsed.systemPrompt, 'You are a Reviewer. Reviews code for bugs and best practices', 'return JSON should include systemPrompt field');
    ...
    test('T16: treats empty string name as missing', async () => {
        const mock = new MockAgentStudioService();
        const result = await handleNewAgentTool({ name: '', role: 'Developer', description: 'Writes code' }, mock);
        const parsed = parseResult(result);
        assert.strictEqual(parsed.success, false);
        assert.ok(parsed.error.toLowerCase().includes('name'));
        assert.strictEqual(mock.createAgentCalls.length, 0);
    ...
    test('T17: slugifies multi-word name with underscores', async () => {
        const mock = new MockAgentStudioService();
        const result = await handleNewAgentTool({ name: 'Senior Backend_Developer', role: 'Developer', description: 'Backend dev' }, mock);
        const parsed = parseResult(result);
