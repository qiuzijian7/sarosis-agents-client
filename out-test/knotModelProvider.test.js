...
suite('Knot AG-UI Model Provider (Phase 3)', () => {
    ...
    class MockKnotModelProvider {
        ...
        onDidChangeAgents = { /* Event */};
        _authStatus = ModelAuthStatus.NotConfigured;
        _agents = [];
        constructor() { }
        getAuthStatus() {
        ...
        async *chat(modelId, messages, options, context) {
            // 模拟 AG-UI 流式响应
    ...
    test('chat method uses agentId from context', async () => {
        ...
        provider.chat = async function* (modelId, messages, options, context) {
