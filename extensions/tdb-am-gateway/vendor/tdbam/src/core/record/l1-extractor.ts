...
import { extractFirstJsonArray, shouldExtractL1 } from "../../utils/sanitize.js";
 ...
 *
 * Uses {@link extractFirstJsonArray} (bracket-depth scan) instead of a greedy
 * regex so that LLM output like:
 *
 *   [{"scene_name":"...", "memories":[...]}]
 *   以上就是我提取的内容，列表如下: [...]
 *
 * cleanly yields the first array and discards the trailing chatter, instead
 * of failing with `Unexpected non-whitespace character after JSON`.
 */
function parseExtractionResult(raw: string, logger?: Logger): SceneSegment[] {
  ...
  if (!extracted.success) {
    ...
    logger?.warn?.(
      `${TAG} [l1-debug] NO_JSON taskId=l1-extraction, reason=${extracted.reason}, ` +
      `rawLen=${raw.length}, rawFull=${JSON.stringify(rawPreview)}` +
      `${raw.length > 2048 ? `…(+${raw.length - 2048})` : ""}`,
    );
    return [];
  }

  if (extracted.trailing.length > 0) {
    // Diagnostic only — the LLM kept talking after the JSON, but we already
    // sliced the structurally-complete array, so this is now harmless.
    const tailPreview = extracted.trailing.slice(0, 240);
    logger?.debug?.(
      `${TAG} parseExtractionResult: ignored ${extracted.trailing.length} trailing char(s) after JSON array. ` +
      `head=${JSON.stringify(tailPreview)}${extracted.trailing.length > 240 ? "…" : ""}`,
    );
  }
  ...
  try {
    ...
    logger?.warn?.(
      ...
      `extractedLen=${extracted.json.length}, head=${JSON.stringify(extracted.json.slice(0, 240))}…`,
    );
    return [];
  }
  ...
  // 防御性检查：如果数组里没有一个元素含有 scene_name 字段，
  // 说明 LLM 输出的是工具调用 JSON（被误解析），而非 scene 数组，直接返回空
  const hasSceneName = parsed.some(
    (item) => item && typeof item === "object" && "scene_name" in (item as object),
  );
  if (parsed.length > 0 && !hasSceneName) {
    logger?.warn?.(
      `${TAG} [l1-debug] LLM output looks like tool-call JSON (no scene_name field found). ` +
      `Discarding ${parsed.length} item(s). LLM likely ignored tool_choice:none constraint.`,
    );
    return [];
  }
