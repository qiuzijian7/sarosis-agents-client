...
import { extractFirstJsonArray } from "../../utils/sanitize.js";
...
function parseBatchResult(
  ...
  // Robust JSON-array extraction: handles trailing chatter, nested arrays,
  // and unescaped control characters inside string literals. See
  // {@link extractFirstJsonArray} for full rationale.
  const extracted = extractFirstJsonArray(raw);
  if (!extracted.success) {
    logger?.warn?.(
      `${TAG} No JSON array found in conflict detection response: reason=${extracted.reason}, ` +
      `rawLen=${raw.length}, head=${JSON.stringify(raw.slice(0, 240))}…`,
    );
    return fallbackStoreAll(memories);
  }
  if (extracted.trailing.length > 0) {
    logger?.debug?.(
      `${TAG} ignored ${extracted.trailing.length} trailing char(s) after JSON array (LLM kept talking)`,
    );
  }
