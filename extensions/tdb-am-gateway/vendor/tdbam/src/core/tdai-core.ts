...
export class TdaiCore {
  ...
  async triggerDistillL2(sessionKey: string): Promise<void> {
    ...
    await this.ensureSchedulerStarted();
    if (!this.scheduler) return;
    this.scheduler.forceL2(sessionKey);
    this.logger.info(`${TAG} triggerDistillL2: session=${sessionKey}`);
  }
  ...
  async triggerDistillL3(): Promise<void> {
    ...
    await this.ensureSchedulerStarted();
    if (!this.scheduler) return;
    this.scheduler.forceL3();
    this.logger.info(`${TAG} triggerDistillL3: triggered`);
  }

  /**
   * Manually re-extract L1 memories for a session by rewinding the L1 cursor
   * and force-running the L1 pipeline. Used to recover from:
   *   - Cursor over-advancement after transient LLM/JSON failures
   *   - Re-running L1 after fixing the extraction prompt
   *   - Backfilling L1 for sessions that have L0 history but no L1 records
   *
   * Behaviour:
   *   1. Rewind `last_l1_cursor` to `sinceMs` (default 0 = full rescan)
   *   2. Invoke the configured L1 runner synchronously (reads from L0, calls LLM, writes L1)
   *   3. Return processed/extracted/stored counts plus the previous cursor value
   *
   * Errors from the underlying L1 runner are propagated — callers (e.g. the
   * /admin/l1/reextract HTTP endpoint) are expected to surface them.
   *
   * @param sessionKey Session to re-extract.
   * @param sinceMs    New cursor value in epoch ms. 0 (default) rescans all L0.
   * @param dryRun     When true, only rewind the cursor without running L1
   *                   (useful for staging the rewind separately from execution).
   */
  async reextractL1(
    sessionKey: string,
    sinceMs: number = 0,
    ...
    if (!sessionKey) {
      throw new Error("reextractL1: sessionKey is required");
    }
    await this.storeReady?.catch(() => { });

    const checkpoint = new CheckpointManager(this.dataDir, this.logger);
    const { previousCursor } = await checkpoint.resetL1Cursor(sessionKey, sinceMs);

    if (dryRun) {
      this.logger.info(
        `${TAG} reextractL1 dry-run session=${sessionKey}: cursor ${previousCursor} → ${sinceMs} (no run)`,
      );
      return {
        sessionKey,
        previousCursor,
    ...
    if (!this.scheduler) {
      throw new Error("reextractL1: pipeline scheduler not initialised (extraction disabled?)");
    }

    // Make sure scheduler is fully wired before forcing the runner.
    await this.ensureSchedulerStarted();

    const result = await this.scheduler.forceRunL1(sessionKey);
    const processedCount = (result && typeof result === "object" && "processedCount" in result)
      ? Number((result as { processedCount: number }).processedCount) || 0
      : 0;

    this.logger.info(
      `${TAG} reextractL1 session=${sessionKey}: cursor ${previousCursor} → ${sinceMs}, processed=${processedCount}`,
    );
