...
export interface VectorSearchResult {
  ...
  /** Source L0 message ids (parsed from JSON column). Empty for legacy rows. */
...
export interface L1QueryFilter {
  ...
  updatedAfter?: string;
  /** If provided, only return records whose scene_name exactly matches this value. */
  sceneName?: string;
  /** If provided, only return records with priority >= this value. */
  minPriority?: number;
  /** Cap the number of returned rows (newest first by updated_time). 0/undefined = unlimited. */
  limit?: number;
}
...
function getJieba(): JiebaInstance | null {
  ...
  //
  // 后续若要接更好的中文分词，建议用 `segmentit`（纯 JS）替换 fallback。详见
  // vendor/tdbam/COPY_MANIFEST.md 第 3 节 Q10 条目。
  _jieba = null; // 标记永远不可用
  return _jieba;
}
 ...
 *     这些在文档"GM代码在S1GameCheatExtension..."中并不存在，导致整体 AND 失败。
 *
 * 解决：按 **连续脚本段** 切分（CJK 段 / ASCII 段 / 数字段），每段独立成 phrase
 * 用 OR 连接。这样：
 *   - 每个段长度更短，trigram 失配概率小
 ...
 *   "S1GameCheatExtension" → '"S1GameCheatExtension"'
 *
 * 例子（jieba 兜底）:
 *   "用户喜欢编程和TypeScript" → '"用户" OR "喜欢" OR "编程" OR "TypeScript"'
 */
export function buildFtsQuery(raw: string): string | null {
  ...
  if (!jieba) {
    ...
    // 中不存在而完全失配。例如 query "GM代码在哪个文件" vs 文档 "GM代码在S1...",
    // query 的 trigram "在哪个" / "哪个文" 在文档里不存在 → AND 失败。
    //
    // 解决：把 query 拆成多个**长度 3-6** 的小段，每个小段独立成 phrase 用 OR
    // 连接。任一小段命中即召回，BM25 让命中更多段的文档排前面。
    //
    // 拆分策略：
    //   1) 按脚本边界（CJK ↔ ASCII）切初步段
    //   2) 对超过 6 字符的 CJK 段，进一步用 3-字符滑动窗口生成 trigram
    //      （ASCII 段保留整体——驼峰英文如 S1GameCheatExtension 整段就是一个 token）
    //   3) 长度 < 3 的段丢弃（trigram 无法生成 token）
    const phrases = new Set<string>();

    const segs = trimmed.match(/[\p{Script=Han}]+|[A-Za-z0-9_]+/gu) ?? [];
    for (const seg of segs) {
      const t = seg.trim().replaceAll('"', "");
      if (t.length < 3) continue;

      // ASCII 段：保留整体（驼峰 / 标识符等）
      const isAscii = /^[A-Za-z0-9_]+$/.test(t);
      if (isAscii) {
        phrases.add(t);
        continue;
      }

      // CJK 段：整段保留 + 3 字符以上做 3-gram 滑窗
      phrases.add(t);
      if (t.length >= 3) {
        const chars = Array.from(t);
        for (let i = 0; i + 3 <= chars.length; i++) {
          phrases.add(chars.slice(i, i + 3).join(""));
        }
      }
    }

    // 额外：对"整段去空格"的字符串也做 3-gram 滑窗，覆盖跨脚本边界的情况。
    // 例如 "GM代码放哪了" → 跨边界 trigram "GM代"/"M代码" 文档里有，能匹配。
    const compactRaw = trimmed.replace(/\s+/g, "").replaceAll('"', "");
    if (compactRaw.length >= 3) {
      const compactChars = Array.from(compactRaw);
      // 限制窗口数量避免 query 过长（>50 trigram 时 OR 表达式可能撑爆 FTS5）
      const maxWindows = Math.min(compactChars.length - 2, 30);
      for (let i = 0; i < maxWindows; i++) {
        phrases.add(compactChars.slice(i, i + 3).join(""));
      }
      // 整段也作为 phrase（短的）
      if (compactRaw.length >= 3 && compactRaw.length <= 30) {
        phrases.add(compactRaw);
      }
    }

    const list = [...phrases].filter((s) => s.length >= 3);
    if (list.length === 0) return null;

    return list.map((t) => `"${t}"`).join(" OR ");
  }

  // jieba 可用路径：保留原 OR-token 拆分行为（跨 host 兼容）
  const tokens = jieba
  ...
  const quoted = dedup.map((t) => `"${t.replaceAll('"', "")}"`);
...
/**
 * Parse the `source_message_ids` JSON column safely.
 * Returns an empty array on null/empty/parse-error so callers can rely on
 * a stable string[] shape.
 */
...
export interface FtsSearchResult {
  ...
  session_id: string;
  metadata_json: string;
  /** Source L0 message ids (enriched from l1_records). Empty for legacy rows. */
  source_message_ids: string[];
}
...
export class VectorStore implements IMemoryStore {
  ...
  init(providerInfo?: EmbeddingProviderInfo): VectorStoreInitResult {
    ...
    if (this.dimensions > 0) {
      ...
      this.logger?.debug?.(
        `${TAG} sqlite-vec load skipped (dimensions=0, vector recall disabled). ` +
        `FTS5 path remains fully functional.`,
      );
    }
  ...
  private initSchema(providerInfo?: EmbeddingProviderInfo): VectorStoreInitResult {
    ...
    // Migration: add source_message_ids column if missing (existing DBs pre-A/D recall)
    try {
      this.db.exec("ALTER TABLE l1_records ADD COLUMN source_message_ids TEXT DEFAULT '[]'");
      this.logger?.debug?.(`${TAG} Migrated l1_records: added source_message_ids column`);
    } catch {
    ...
    this.stmtUpsertMeta = this.db.prepare(`
      INSERT INTO l1_records (
        ...
        created_time, updated_time, metadata_json, source_message_ids
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    try {
      ...
      // L1 FTS5 virtual table (v3 schema — trigram tokenizer)
      //
      // Bug fix（CJK 召回失效）：
      //   v2 默认 `simple` tokenizer 不识别 CJK，超长中英混合字符串被索引为单 token。
      //   `unicode61` 加 categories 同样不行——它只把 ASCII 标点和空格当分隔符，
      //   汉字之间仍然连成一个超长 token（实测："GM代码在S1GameCheatExtension"
      //   整个成为单 token）。
      //
      // 修复：改用 SQLite 3.34+ 内建的 `trigram` tokenizer，它会生成
      //   所有连续 3 字符的 substring 作为 token，对中英混合、不分词
      //   场景特别友好。查询会被转成 LIKE，不需 buildFtsQuery 预处理。
      //   - case_sensitive=0 让 GM/gm 互匹配
      //   - remove_diacritics=1 允许重音变体匹配（对中文无影响）
      //
      // tokenizer 变动按 schema 变动处理，调 migrateFtsTablesIfNeeded 调识别重建。
      this.db.exec(`
      ...
      // L0 FTS5 virtual table (v3 schema — trigram tokenizer)
      this.db.exec(`
    ...
    try {
      ...
      for (const { record_id, distance } of rows) {
        ...
        results.push({
          ...
          source_message_ids: parseSourceMessageIds(meta.source_message_ids),
        });
      }
  ...
  queryL1Records(filter?: L1QueryFilter): L1RecordRow[] {
    ...
    try {
      const { sessionKey, sessionId, updatedAfter, sceneName, minPriority, limit } = filter ?? {};

      let raw: Record<string, unknown>[];

      // When the new optional filters (sceneName / minPriority / limit) are used,
      // build a dynamic statement instead of going through the prepared-statement
      // matrix. The pre-existing prepared statements remain the hot path for the
      // legacy session*/updatedAfter triplet.
      const usesExtended = sceneName != null || minPriority != null || (limit != null && limit > 0);

      if (usesExtended) {
        const wheres: string[] = [];
        const args: Array<string | number> = [];
        if (sessionId) { wheres.push("session_id = ?"); args.push(sessionId); }
        if (sessionKey) { wheres.push("session_key = ?"); args.push(sessionKey); }
        if (updatedAfter) { wheres.push("updated_time > ?"); args.push(updatedAfter); }
        if (sceneName) { wheres.push("scene_name = ?"); args.push(sceneName); }
        if (minPriority != null) { wheres.push("priority >= ?"); args.push(minPriority); }
        const whereClause = wheres.length > 0 ? `WHERE ${wheres.join(" AND ")}` : "";
        // Newest-first ordering matches the "recent N" semantics expected by recall;
        // callers that want chronological order can re-sort client-side.
        const limitClause = (limit != null && limit > 0) ? `LIMIT ${Math.floor(limit)}` : "";
        const cols = `record_id, content, type, priority, scene_name, session_key, session_id,
          timestamp_str, timestamp_start, timestamp_end,
          created_time, updated_time, metadata_json, source_message_ids`;
        const sql = `SELECT ${cols} FROM l1_records ${whereClause} ORDER BY updated_time DESC ${limitClause}`.replace(/\s+/g, " ").trim();
        raw = this.db.prepare(sql).all(...args) as Record<string, unknown>[];
      ...
      // Parse source_message_ids JSON column into string[] before exposing to callers.
      const rows: L1RecordRow[] = raw.map((r) => ({
        ...
        timestamp_str: r.timestamp_str as string,
        timestamp_start: r.timestamp_start as string,
        timestamp_end: r.timestamp_end as string,
        created_time: r.created_time as string,
        updated_time: r.updated_time as string,
        metadata_json: r.metadata_json as string,
        source_message_ids: parseSourceMessageIds(r.source_message_ids as string | undefined),
      }));

      this.logger?.info(
        `${TAG} [L1-query] filter={sessionKey=${sessionKey ?? "(all)"}, sessionId=${sessionId ?? "(all)"}, ` +
        `updatedAfter=${updatedAfter ?? "(none)"}, sceneName=${sceneName ?? "(any)"}, ` +
        `minPriority=${minPriority ?? "(none)"}, limit=${limit ?? "(none)"}}, ` +
  ...
  /**
   * Fetch L0 messages by their record_id. Order of returned rows mirrors the
   * order of ids supplied by the caller; missing ids are silently skipped.
   *
   * **Fault-tolerant**: returns an empty array on any error.
   */
  getL0ByIds(recordIds: string[]): Array<{
    record_id: string;
    session_key: string;
    session_id: string;
    role: string;
    ...
    try {
      ...
      // Re-order to match input id order so callers get a stable shape.
      ...
      this.logger?.warn(
        `${TAG} [L0-getByIds] FAILED (non-fatal, returning empty): ${err instanceof Error ? err.message : String(err)}`,
      );
      return [];
    }
  }
  ...
  /**
   * Fetch L0 messages within a sessionKey + recorded_at window.
   * Returned rows are in chronological order (oldest first).
   *
   * **Fault-tolerant**: returns an empty array on any error.
   */
  queryL0InWindow(sessionKey: string, fromIso: string, toIso: string, limit = 20): Array<{
    record_id: string;
    session_key: string;
    session_id: string;
    role: string;
    ...
    try {
      const rows = this.db
        .prepare(`
          SELECT record_id, session_key, session_id, role, message_text, recorded_at, timestamp
          FROM l0_conversations
          WHERE session_key = ? AND recorded_at >= ? AND recorded_at <= ?
          ORDER BY recorded_at ASC
          LIMIT ?
        `)
        .all(sessionKey, fromIso, toIso, Math.max(1, Math.floor(limit))) as Array<{
          record_id: string;
          session_key: string;
          session_id: string;
          role: string;
      ...
      this.logger?.warn(
        `${TAG} [L0-queryInWindow] FAILED (non-fatal, returning empty): ${err instanceof Error ? err.message : String(err)}`,
      );
      return [];
    }
  }
  ...
  /**
   * Get all L0 records with full metadata (role, session_key, timestamp, ...).
   *
   * This is the richer companion to {@link getAllL0Texts}, used by inspection
  ...
  searchL1Fts(ftsQuery: string, limit = 20): FtsSearchResult[] {
    ...
    try {
      ...
      // FTS5 virtual table doesn't carry source_message_ids (would force a
      // disruptive DROP+rebuild migration). Enrich each hit with a single
      // batched lookup against l1_records.
      const ids = rows.map((r) => r.record_id);
      const placeholders = ids.map(() => "?").join(",");
      const enrichRows = this.db
        .prepare(`SELECT record_id, source_message_ids FROM l1_records WHERE record_id IN (${placeholders})`)
        .all(...ids) as Array<{ record_id: string; source_message_ids: string }>;
      const idToSourceIds = new Map<string, string[]>();
      for (const er of enrichRows) {
        idToSourceIds.set(er.record_id, parseSourceMessageIds(er.source_message_ids));
      }
      ...
      return rows.map((r) => ({
        ...
        source_message_ids: idToSourceIds.get(r.record_id) ?? [],
      }));
    } catch (err) {
      this.logger?.warn(
  ...
  private migrateFtsTablesIfNeeded(): boolean {
    try {
      ...
      const l1Exists = this.db
        ...
        .get();
      if (!l1Exists) {
        // Fresh install — tables will be created with v3 schema.
      ...
      if (!hasV2Col) {
        // v1 → v3: drop both FTS tables (data will be repopulated by rebuildFtsIndex)
        this.logger?.info(`${TAG} Migrating FTS5 tables from v1 to v3 (unicode61 tokenizer)`);
        this.db.exec("DROP TABLE IF EXISTS l1_fts");
        this.db.exec("DROP TABLE IF EXISTS l0_fts");
        return true;
      }

      // v2/v3-unicode61 → v3-trigram 检测：tokenizer 变化。
      //
      ...
      const masterRow = this.db
        .prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='l1_fts'")
        .get() as { sql: string } | undefined;
      const sqlText = masterRow?.sql ?? "";
      const usesTrigram = /tokenize\s*=\s*["']?trigram/i.test(sqlText);

      if (usesTrigram) {
        return false; // Already v3-trigram — no migration needed
      }

      // 需要迁移到 trigram（无论是 v2 simple 还是 v3 unicode61）
      this.logger?.info(`${TAG} Migrating FTS5 tables to trigram tokenizer (CJK-friendly without segmenter)`);
      this.db.exec("DROP TABLE IF EXISTS l1_fts");
      this.db.exec("DROP TABLE IF EXISTS l0_fts");
      return true;
    } catch (err) {
