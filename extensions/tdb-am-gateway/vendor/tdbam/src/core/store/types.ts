...
export interface L1SearchResult {
  ...
  /**
   * Source L0 message ids that contributed to this L1 memory.
   * Empty array for legacy L1 records persisted before this column was added —
   * callers should fall back to time-window lookup in that case.
   */
...
export interface L1FtsResult {
  ...
  session_id: string;
  metadata_json: string;
  /** Same semantics as L1SearchResult.source_message_ids. */
  source_message_ids: string[];
}
...
export interface L1QueryFilter {
  ...
  updatedAfter?: string;
  /** Only return records whose scene_name exactly matches this value. */
  sceneName?: string;
  /** Only return records with priority >= this value. */
  minPriority?: number;
...
export interface IMemoryStore {
  ...
  /**
   * Fetch L0 messages by their record_id.
   * Used by recall path to enrich L1 hits with their source L0 context.
   * Returns rows in the same order as the input ids; missing ids are silently skipped.
   */
  getL0ByIds?(recordIds: string[]): MaybePromise<L0QueryRow[]>;

  /**
   * Fetch L0 messages within a session_key + recorded_at time window.
   * Used as a fallback when an L1 hit has no source_message_ids (legacy data).
   *
   * @param sessionKey  Session key to filter on.
   * @param fromIso     Inclusive lower bound (ISO 8601 recorded_at).
   * @param toIso       Inclusive upper bound (ISO 8601 recorded_at).
   * @param limit       Max rows; ordered chronologically (oldest first).
   */
  queryL0InWindow?(sessionKey: string, fromIso: string, toIso: string, limit?: number): MaybePromise<L0QueryRow[]>;
