...
export class MemoryPipelineManager {
  ...
  /**
   * Force-run the L1 runner for a session **immediately and synchronously**,
   * bypassing buffer/threshold/idle gates. Used by manual re-extraction
   * (e.g. POST /admin/l1/reextract) after rewinding the L1 cursor.
   *
   * This delegates to the configured L1 runner which is responsible for
   * reading L0 messages from the cursor onward, so the in-memory buffer
   * can be empty without affecting the result.
   *
   * Returns whatever the underlying L1 runner returns (or undefined if
   * no runner is wired). Errors are propagated to the caller — unlike
   * the auto-pipeline path, we want manual triggers to surface failures.
   */
  async forceRunL1(sessionKey: string): Promise<L1RunnerResult | void> {
    if (!this.l1Runner) {
      throw new Error("forceRunL1: no L1 runner wired");
    }
    return this.l1Runner({ sessionKey, msg: [], bg_msg: [] });
  }
  ...
  // ============================
  // Public: force-trigger L2 / L3 (for /distill endpoints)
  // ============================
  ...
  forceL2(sessionKey: string): void {
    ...
    const state = this.getOrCreateState(sessionKey);
    // 确保 l2_pending_l1_count > 0，否则 runL2 内部会跳过
    if (state.l2_pending_l1_count === 0) {
      state.l2_pending_l1_count = Math.max(state.conversation_count, 1);
    }

    this.logger?.debug?.(`${TAG} [${sessionKey}] forceL2: enqueuing L2 immediately`);

    const timers = this.getOrCreateTimers(sessionKey);
    if (timers.l2Queued) {
      this.logger?.debug?.(`${TAG} [${sessionKey}] forceL2: L2 already queued, skipping`);
      return;
    }
    timers.l2Queued = true;
    ...
    this.l2Queue.add(async () => {
      ...
      timers.l2Queued = false;
      this.logger?.error(
        `${TAG} [${sessionKey}] forceL2 task failed: ${err instanceof Error ? err.message : String(err)}`,
      );
    });
  }
  ...
  forceL3(): void {
    if (this.destroyed) return;
    this.logger?.debug?.(`${TAG} forceL3: triggering L3 immediately`);
    this.triggerL3();
  }
}
