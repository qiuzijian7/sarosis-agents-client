...
export class CheckpointManager {
  ...
  /**
   * Reset (rewind) the L1 cursor for a session so that subsequent runL1 calls
   * will re-process L0 messages that occurred after `sinceMs`. Use cases:
   *   - Manual re-extraction after fixing the L1 prompt or JSON parser
   *   - Recovery from transient LLM failures that left cursor over-advanced
   *   - Debugging: full re-scan with sinceMs=0
   *
   * Also clears `last_scene_name` so the first re-extracted batch starts a
   * fresh scene chain (otherwise the dedup/scene-continuity logic would
   * reference a scene that may already cover the rewound region).
   *
   * @param sessionKey Session to reset.
   * @param sinceMs    New cursor value in epoch ms. 0 (default) = full rescan.
   */
  async resetL1Cursor(sessionKey: string, sinceMs: number = 0): Promise<{ previousCursor: number; previousScene: string }> {
    let previousCursor = 0;
    let previousScene = "";
    await this.mutate((cp) => {
      const state = this.getRunnerState(cp, sessionKey);
      previousCursor = state.last_l1_cursor;
      previousScene = state.last_scene_name ?? "";
      state.last_l1_cursor = sinceMs;
      state.last_scene_name = "";
    });
    this.logger.info(
      `[checkpoint] resetL1Cursor session=${sessionKey}: ` +
      `cursor ${previousCursor} → ${sinceMs}, scene "${previousScene}" → "" (cleared)`,
    );
    return { previousCursor, previousScene };
  }
