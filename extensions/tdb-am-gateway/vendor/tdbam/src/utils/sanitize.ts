...
export function sanitizeText(text: string): string {
  ...
  cleaned = cleaned.replace(/<related-context>[\s\S]*?<\/related-context>/g, "");
...
export function escapeXmlTags(text: string): string {
  ...
  return text.replace(
    /<\/?(?:user-persona|relevant-memories|related-context|scene-navigation|relevant-scenes|memory-tools-guide|system|assistant)>/gi,
...
/**
 * Result of {@link extractFirstJsonArray}.
 *
 * - On success, `json` is a sanitized JSON-array string ready to feed to
 *   `JSON.parse`. `trailing` (when non-empty) is what came after the array
 *   in the original input — useful for diagnostics ("the LLM kept talking
 *   after the JSON").
 * - On failure, `reason` describes why extraction failed. The caller should
 *   log it and fall back to its empty-result behaviour.
 */
export type ExtractJsonArrayResult =
  | { readonly success: true; readonly json: string; readonly startIndex: number; readonly endIndex: number; readonly trailing: string }
  | { readonly success: false; readonly reason: string };

/**
 * Robustly extract the *first complete* top-level JSON array from a raw LLM
 * output and return it sanitized for `JSON.parse`.
 *
 * Why this exists
 * ───────────────
 * The previous implementation used `raw.match(/\[[\s\S]*\]/)`, which is
 * greedy: it grabs from the first `[` to the *last* `]` in the entire
 * string. When the LLM keeps talking after a structurally complete JSON
 * array — typical for chat-tuned models such as Knot's AG-UI agents which
 * append explanations like "以上就是我提取的内容..." plus markdown lists
 * containing more `[ ]` pairs — the regex captures one giant slab that
 * is no longer valid JSON. `JSON.parse` then dies with errors like
 * `Unexpected non-whitespace character after JSON at position 8195`.
 *
 * Approach
 * ────────
 * 1. Strip surrounding markdown code fences (```json ... ``` / ``` ... ```).
 * 2. Scan forward for the first `[`.
 * 3. Walk character-by-character with a single-counter bracket depth tracker,
 *    correctly skipping over:
 *      - JSON string literals (delimited by unescaped `"`)
 *      - escape sequences inside strings (`\"`, `\\`, etc.)
 *      - nested `[]` pairs (depth + 1 / depth - 1)
 *    Stop the moment depth returns to 0 — that is the end of the *first*
 *    complete top-level array. Anything after it is treated as trailing
 *    chatter and ignored.
 * 4. Pass the clean substring through {@link sanitizeJsonForParse} to fix
 *    raw control characters inside string literals (the original sanitize
 *    logic).
 *
 * The returned `endIndex` points one past the closing `]` in the *trimmed*
 * input; combine with `startIndex` to slice. `trailing` is the substring
 * after `endIndex` (already trimmed).
 *
 * @param raw       Raw LLM response text.
 * @returns         Discriminated-union result; never throws.
 */
export function extractFirstJsonArray(raw: string): ExtractJsonArrayResult {
  if (!raw || typeof raw !== "string") {
    return { success: false, reason: "empty input" };
  }

  // --- Step 1: strip surrounding markdown code fences -------------------
  ...
  if (startIndex < 0) {
    return { success: false, reason: "no '[' found in response" };
  }

  // --- Step 3: bracket-depth scan, skipping strings ---------------------
  let depth = 0;
  let inString = false;
  let endIndex = -1;
  ...
  for (let i = startIndex; i < cleaned.length; i++) {
    ...
    if (inString) {
      if (ch === "\\") {
        // Skip the escaped char (handles \", \\, \n, \uXXXX prefix start, etc.).
        // We don't need to fully decode \uXXXX — just hop one char forward, the
        // remaining hex digits will pass through as ordinary in-string content.
        i++;
        continue;
      }
      if (ch === '"') {
    ...
    if (ch === "]") {
      depth--;
      if (depth === 0) {
        endIndex = i + 1; // one past the closing bracket
        break;
      }
      if (depth < 0) {
        // Unbalanced — should never happen for well-formed JSON, but bail
        // out gracefully rather than silently include garbage.
        return { success: false, reason: `unbalanced ']' at offset ${i}` };
      }
    }
  }

  if (endIndex < 0) {
    return { success: false, reason: `no matching ']' for '[' at offset ${startIndex} (depth still ${depth} at EOF)` };
  }

  // --- Step 4: sanitize control chars inside string literals ------------
  const slice = cleaned.slice(startIndex, endIndex);
  const sanitized = sanitizeJsonForParse(slice);
  const trailing = cleaned.slice(endIndex).trim();
