...
export function createL1Runner(opts: {
  ...
  return async ({ sessionKey }) => {
    ...
    try {
      ...
      //
      // Cursor advancement policy (revised):
      //   - storedCount > 0  → advance cursor (normal success path)
      //   - storedCount = 0  → KEEP cursor (do NOT advance) so the next pass can
      //                        re-attempt these L0 messages. This prevents the
      //                        "L1 silently empty forever" bug where transient
      //                        LLM/JSON failures permanently skipped messages.
      //
      // Trade-off: a conversation that genuinely has nothing memory-worthy
      // (e.g. pure pleasantries) will be re-scanned on every L1 trigger.
      // We accept this — the LLM call is cheap relative to losing real memories.
      if (totalStored > 0) {
        await checkpoint.markL1ExtractionComplete(sessionKey, totalStored, maxRecordedAtMs || undefined, lastSceneName);
        logger.info(
          `${TAG} [l1] L1 complete: extracted=${totalExtracted}, stored=${totalStored} (${groups.length} group(s)), cursor advanced to ${maxRecordedAtMs}`,
        );
      } else {
        logger.warn(
          `${TAG} [l1] L1 stored 0 memories (extracted=${totalExtracted}, ${groups.length} group(s), ${totalMessages} msgs scanned) — KEEPING cursor for retry. ` +
          `If this persists, check LLM JSON output / quality gate / dedup logic.`,
        );
      }
