 ...
 *    saros renderer ESM 环境加载（试过显式 import('vscode') 也失败）。
 *
 *    现在的架构是：
 *      saros Electron 主进程 → spawn extensions/tdb-am-gateway/host/host.mjs
 *                                  → 子进程内加载 vendor TdaiGateway
 *                                  → 监听 127.0.0.1:8420 (HTTP)
 *      saros renderer 各扩展 → fetch http://127.0.0.1:8420/* 访问
 *
 *  本文件保留为"占位 stub"：
 *    - 只在被 capability framework 加载时打个日志、宣称就绪
 *    - 不做任何 vendor 加载、不依赖 vscode/fs/path/url
 *    - 如果 saros 主进程 spawn 失败，gateway 端口 fetch 会自然失败，
...
/** saros capability framework 期望 default export 是一个有 activate/deactivate 的类。 */
