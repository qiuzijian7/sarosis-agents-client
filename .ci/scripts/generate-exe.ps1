...
# EndUpdateResource failed (110)）。仅删除 Inno 产物 VsSarosisSetup*.exe，保留暂存的应用本体 (Code.exe 等)。
Get-ChildItem -Path .build\win32-x64 -Recurse -Filter VsSarosisSetup*.exe -EA SilentlyContinue | Remove-Item -Force -EA SilentlyContinue
