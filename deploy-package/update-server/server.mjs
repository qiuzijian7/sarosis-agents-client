 ...
 *   - quality  例：sarosis（与 product.json 的 quality 一致）
 ...
 *
 * 运行：
 *   node build/sarosis/update-server/server.mjs
 *   PORT=8080 node build/sarosis/update-server/server.mjs
 *   GH_REPO=qiuzijian7/sarosis-agents-client node build/sarosis/update-server/server.mjs
 *
...
async function readFromGitHub(platform) {
	...
	const headers = {
		...
		'User-Agent': 'vssarosis-update-server'
	};
	if (GH_TOKEN) {
...
const server = http.createServer(async (req, res) => {
	...
	if (url.pathname === '/' || url.pathname === '/health') {
		...
		res.end(JSON.stringify({ ok: true, service: 'vssarosis-update-server', mode: GH_REPO ? 'github' : 'manifest' }));
		return;
	}
...
server.listen(PORT, () => {
	...
	console.log(`测试: curl http://localhost:${PORT}/api/update/win32-x64-user/sarosis/0000000000000000000000000000000000000000`);
});
