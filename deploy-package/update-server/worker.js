 ...
 *   3. wrangler deploy
 *   4. 把部署后得到的 URL（如 https://vssarosis-update.xxx.workers.dev）
 ...
 *   [vars]
 *   GH_REPO = "qiuzijian7/sarosis-agents-client"
...
export default {
	...
	async fetch(request, env) {
		...
		if (url.pathname === '/' || url.pathname === '/health') {
			return Response.json({ ok: true, service: 'vssarosis-update-worker', repo: env.GH_REPO });
		}
		...
		const headers = {
			...
			'User-Agent': 'vssarosis-update-worker'
		};
		if (env.GH_TOKEN) {
