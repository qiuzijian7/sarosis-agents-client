...
class AutorunObserver {
    ...
    constructor(_debugNameData, _runFn, _changeTracker, debugLocation) {
        ...
        (0, logging_js_1.getLogger)()?.handleAutorunCreated(this, debugLocation);
        this._run();
    ...
    dispose() {
        ...
        (0, logging_js_1.getLogger)()?.handleAutorunDisposed(this);
    ...
    _run() {
        ...
        try {
            if (!this._disposed) {
                (0, logging_js_1.getLogger)()?.handleAutorunStarted(this);
    ...
    endUpdate(_observable) {
        ...
        (0, deps_js_1.assertFn)(() => this._updateCount >= 0);
    }
    ...
    handleChange(observable, change) {
        if (this._isDependency(observable)) {
            (0, logging_js_1.getLogger)()?.handleAutorunDependencyChanged(this, observable, change);
            try {
    ...
    get store() {
        this._ensureNoRunning();
        if (this._disposed) {
            throw new deps_js_1.BugIndicatingError('Cannot access store after dispose');
        }
        if (this._store === undefined) {
            this._store = new deps_js_1.DisposableStore();
        }
        return this._store;
    }
    _delayedStore = undefined;
    get delayedStore() {
        this._ensureNoRunning();
        if (this._disposed) {
            throw new deps_js_1.BugIndicatingError('Cannot access store after dispose');
        ...
        if (this._delayedStore === undefined) {
            this._delayedStore = new deps_js_1.DisposableStore();
        }
    ...
    _checkIterations() {
        if (this._iteration > 100) {
            (0, deps_js_1.onBugIndicatingError)(new deps_js_1.BugIndicatingError(`Autorun '${this.debugName}' is stuck in an infinite update loop.`));
            return true;
        }
        return false;
    }
