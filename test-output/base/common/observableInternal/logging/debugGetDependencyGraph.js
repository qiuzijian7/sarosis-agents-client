...
exports.debugGetObservableGraph = debugGetObservableGraph;
...
function formatObservableInfoWithDependencies(info, indentLevel, alreadyListed, options) {
    ...
    lines.push(`${indent}  value: ${(0, consoleObservableLogger_js_1.formatValue)(info.value, 50)}`);
...
class Info {
    ...
    static from(obs, debugNamePostProcessor) {
        if (obs instanceof autorunImpl_js_1.AutorunObserver) {
        ...
        else if (obs instanceof derivedImpl_js_1.Derived) {
            const state = obs.debugGetState();
