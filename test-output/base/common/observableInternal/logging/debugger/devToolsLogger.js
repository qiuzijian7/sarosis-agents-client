...
const autorunImpl_js_1 = require("../../reactions/autorunImpl.js");
const consoleObservableLogger_js_1 = require("../consoleObservableLogger.js");
const debuggerRpc_js_1 = require("./debuggerRpc.js");
const utils_js_1 = require("./utils.js");
const types_js_1 = require("../../../types.js");
const observableFromEvent_js_1 = require("../../observables/observableFromEvent.js");
const errors_js_1 = require("../../../errors.js");
const derivedImpl_js_1 = require("../../observables/derivedImpl.js");
...
class DevToolsLogger {
    ...
    _instanceInfos = new WeakMap();
    _aliveInstances = new Map();
    _activeTransactions = new Set();
    _channel = (0, debuggerRpc_js_1.registerDebugChannel)('observableDevTools', () => {
        return {
            notifications: {
            ...
            requests: {
                ...
                getObservableValueInfo: instanceId => {
                    ...
                    return {
                        observers: [...obs.debugGetObservers()].map(d => this._formatObserver(d)).filter(types_js_1.isDefined),
                    };
                },
                getDerivedInfo: instanceId => {
                    const d = this._aliveInstances.get(instanceId);
                    return {
                        dependencies: [...d.debugGetState().dependencies].map(d => this._formatObservable(d)).filter(types_js_1.isDefined),
                        observers: [...d.debugGetObservers()].map(d => this._formatObserver(d)).filter(types_js_1.isDefined),
                    };
                },
                getAutorunInfo: instanceId => {
                    const obs = this._aliveInstances.get(instanceId);
                    return {
                        dependencies: [...obs.debugGetState().dependencies].map(d => this._formatObservable(d)).filter(types_js_1.isDefined),
                    };
                },
                getTransactionState: () => {
                ...
                setValue: (instanceId, jsonValue) => {
                    const obs = this._aliveInstances.get(instanceId);
                    if (obs instanceof derivedImpl_js_1.Derived) {
                        obs.debugSetValue(jsonValue);
                    }
                    else if (obs instanceof observableValue_js_1.ObservableValue) {
                        obs.debugSetValue(jsonValue);
                    }
                    else if (obs instanceof observableFromEvent_js_1.FromEventObservable) {
                        obs.debugSetValue(jsonValue);
                    }
                    else {
                        throw new errors_js_1.BugIndicatingError('Observable is not supported');
                    }
                ...
                getValue: instanceId => {
                    const obs = this._aliveInstances.get(instanceId);
                    if (obs instanceof derivedImpl_js_1.Derived) {
                        return (0, consoleObservableLogger_js_1.formatValue)(obs.debugGetState().value, 200);
                    }
                    else if (obs instanceof observableValue_js_1.ObservableValue) {
                        return (0, consoleObservableLogger_js_1.formatValue)(obs.debugGetState().value, 200);
                    }
                ...
                rerun: (instanceId) => {
                    ...
                    else if (obs instanceof autorunImpl_js_1.AutorunObserver) {
                        obs.debugRerun();
                    }
                    else {
                        throw new errors_js_1.BugIndicatingError('Observable is not supported');
    ...
    _getObservableInfo(observable) {
        ...
        if (!info) {
            (0, errors_js_1.onUnexpectedError)(new errors_js_1.BugIndicatingError('No info found'));
            return undefined;
        }
        return info;
    }
    ...
    _getInfo(observer, queue) {
        if (observer instanceof derivedImpl_js_1.Derived) {
            ...
            const changedDependencies = [...info.changedObservables].map(o => this._instanceInfos.get(o)?.instanceId).filter(types_js_1.isDefined);
        ...
        else if (observer instanceof autorunImpl_js_1.AutorunObserver) {
    ...
    constructor() {
        debugLocation_js_1.DebugLocation.enable();
    }
    _pendingChanges = null;
    _changeThrottler = new utils_js_1.Throttler();
    _fullState = {};
    _handleChange(update) {
        (0, utils_js_1.deepAssignDeleteNulls)(this._fullState, update);
        if (this._pendingChanges === null) {
            this._pendingChanges = update;
        }
        else {
            (0, utils_js_1.deepAssign)(this._pendingChanges, update);
        }
    ...
    handleOnListenerCountChanged(observable, newCount) {
        ...
        if (info.listenerCount === 0 && newCount > 0) {
            const type = observable instanceof derivedImpl_js_1.Derived ? 'observable/derived' : 'observable/value';
    ...
    handleObservableUpdated(observable, changeInfo) {
        if (observable instanceof derivedImpl_js_1.Derived) {
        ...
        if (info) {
            if (changeInfo.didChange) {
                info.lastValue = (0, consoleObservableLogger_js_1.formatValue)(changeInfo.newValue, 30);
    ...
    _handleDerivedRecomputed(observable, changeInfo) {
        ...
        const formattedValue = (0, consoleObservableLogger_js_1.formatValue)(changeInfo.newValue, 30);
        info.updateCount++;
        info.changedObservables.clear();
