...
function _isBufferLeakWarningEnabled() {
    return !!process_js_1.env['VSCODE_DEV'];
}
var Event;
(function (Event) {
    ...
    function buffer(event, debugName, flushAfterTimeout = false, _buffer = [], disposable) {
        ...
        if (_isBufferLeakWarningEnabled()) {
            ...
            if (disposable) {
                disposable.add((0, lifecycle_js_1.toDisposable)(() => clearTimeout(bufferLeakWarningData.timerId)));
            }
        }
...
class Emitter {
    ...
    constructor(options) {
        ...
        this._leakageMon = (_globalLeakWarningThreshold > 0 || this._options?.leakWarningThreshold)
            ? new LeakageMonitor(options?.onListenerError ?? errors_js_1.onUnexpectedError, this._options?.leakWarningThreshold ?? _globalLeakWarningThreshold, this._options?.leakWarningName) :
            undefined;
