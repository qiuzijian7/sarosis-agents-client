...
exports.installFakeRunWhenIdle = installFakeRunWhenIdle;
exports.retry = retry;
...
function createCancelablePromise(callback) {
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        Promise.resolve(thenable).then(value => {
            ...
            else if ((0, lifecycle_js_1.isDisposable)(value)) {
...
class Throttler {
    ...
    constructor() {
        ...
        this.cancellationTokenSource = new cancellation_js_1.CancellationTokenSource();
    }
    ...
    clearPending() {
        ...
        for (const task of tasks) {
            if (task.setUndefinedWhenCleared) {
                ...
                task.deferred.error(new errors_js_1.CancellationError());
            }
        }
    }
}
...
class ThrottledWorker extends lifecycle_js_1.Disposable {
    ...
    handler;
    pendingWork = [];
    throttler = this._register(new lifecycle_js_1.MutableDisposable());
    disposed = false;
    lastExecutionTime = 0;
...
function installFakeRunWhenIdle(fakeImpl) {
    const origRunWhenIdle = exports._runWhenIdle;
    const origRunWhenGlobalIdle = exports.runWhenGlobalIdle;
    exports._runWhenIdle = fakeImpl;
    exports.runWhenGlobalIdle = (runner, timeout) => fakeImpl(globalThis, runner, timeout);
    return (0, lifecycle_js_1.toDisposable)(() => {
        exports._runWhenIdle = origRunWhenIdle;
        exports.runWhenGlobalIdle = origRunWhenGlobalIdle;
    });
}
...
class LazyStatefulPromise {
    _compute;
    _promise = new lazy_js_1.Lazy(() => new StatefulPromise(this._compute()));
...
class ProducerConsumer {
    ...
    _ensureNoFinalValue() {
        if (this._finalValue) {
            throw new errors_js_1.BugIndicatingError('ProducerConsumer: cannot produce after final value has been set');
        }
    }
...
class AsyncIterableProducer {
    ...
    static EMPTY = AsyncIterableProducer.fromArray([]);
    static map(iterable, mapFn) {
        return new AsyncIterableProducer(async (emitter) => {
            for await (const item of iterable) {
                emitter.emitOne(mapFn(item));
...
exports.CancelableAsyncIterableProducer = CancelableAsyncIterableProducer;
//#endregion
exports.AsyncReaderEndOfStream = Symbol('AsyncReaderEndOfStream');
class AsyncReader {
    _source;
    _buffer = [];
    _atEnd = false;
    get endOfStream() { return this._buffer.length === 0 && this._atEnd; }
    _extendBufferPromise;
    constructor(_source) {
        this._source = _source;
    }
    async read() {
        ...
        if (this._buffer.length === 0) {
            return exports.AsyncReaderEndOfStream;
        }
    ...
    async readWhile(predicate, callback) {
        do {
            ...
            if (piece === exports.AsyncReaderEndOfStream) {
                break;
            }
            if (!predicate(piece)) {
                break;
    ...
    peekBufferedOrThrow() {
        if (this._buffer.length === 0) {
            ...
            throw new errors_js_1.BugIndicatingError('No buffered elements');
        }
        return this._buffer[0];
    }
...
function createTimeout(ms, cb) {
    ...
    return (0, lifecycle_js_1.toDisposable)(() => clearTimeout(t));
}
