...
export type MarkdownActionHandler = (
	linkContent: string,
...
function preprocessMarkdownString(markdown: IMarkdownString) {
	...
	// Remove <body> wrapper tags added by ChatContentMarkdownRenderer to prevent
	// marked.js from parsing the entire content as an HTML block, which breaks
	// markdown rendering (tables, headers, bold, etc.).
	const bodyMatch = value.match(/^<body>[\s\S]*?<\/body>$/);
	if (bodyMatch) {
		value = value.replace(/^<body>\s*/, "").replace(/\s*<\/body>$/, "");
	}
...
export const allowedMarkdownHtmlAttributes = Object.freeze<
	Array<string | domSanitize.SanitizeAttributeRule>
>([
	"align",
	"autoplay",
	"alt",
		...
		shouldKeep: (element, data) => {
			if (element.tagName === "SPAN") {
				if (data.attrName === "class") {
					return /^codicon codicon-[a-z\-]+( codicon-modifier-[a-z\-]+)?$/.test(
						data.attrValue,
					);
				}
			}
...
function getDomSanitizerConfig(
	...
	return {
		...
		allowedAttributes: {
			override:
				options.allowedAttributes?.override ?? allowedMarkdownHtmlAttributes,
		},
...
function createPlainTextRenderer(): marked.Renderer {
	...
	renderer.table = function ({ header, rows }: marked.Tokens.Table): string {
		return (
			...
			"\n" +
			rows
				.map((cells) => cells.map((cell) => this.tablecell(cell)).join(" "))
				.join("\n") +
			"\n"
		);
	};
		...
		if (subtoken.type === "text") {
			...
			if (lastLine.includes("`")) {
				...
				(hasStartOfLinkTargetAndNoLinkText(lastLine) &&
					token.tokens
						.slice(0, i)
						.some((t) => t.type === "text" && t.raw.match(/\[[^\]]*$/)))
			) {
	...
	*/
	...
	const listEndsInHeading = (list: marked.Tokens.List): boolean => {
		...
		return (
			...
			(lastToken?.type === "list" &&
				listEndsInHeading(lastToken as marked.Tokens.List))
		);
	};
...
function fillInIncompleteTokensOnce(
	...
	if (!newTokens && lastToken?.type === "list") {
		const newListToken = completeListItemPattern(
			lastToken as marked.Tokens.List,
		);
		if (newListToken) {
	...
	if (!newTokens && lastToken?.type === "paragraph") {
		...
		const newToken = completeSingleLinePattern(
			lastToken as marked.Tokens.Paragraph,
		);
		if (newToken) {
			newTokens = [newToken];
			i = tokens.length - 1;
	...
	if (lastToken?.type === "heading") {
		const completeTokens = completeHeading(
			lastToken as marked.Tokens.Heading,
			mergeRawTokenText(tokens),
		);
		if (completeTokens) {
