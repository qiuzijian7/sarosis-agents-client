...
const responsePartSchema = Adapt.v<
	...
	SerializedChatResponsePart
>(
	(obj): SerializedChatResponsePart =>
		obj.kind === "markdownContent" ? obj.content : toJson(obj),
	(a, b) => {
...
const messageSchema = Adapt.object<IParsedChatRequest, IParsedChatRequest>({
	...
	parts: Adapt.v(
		(m) => m.parts,
		(a, b) =>
			a.length === b.length && a.every((part, i) => part.text === b[i].text),
	),
});
...
>({
	variables: Adapt.t(
		(v) => v.variables.map(IChatRequestVariableEntry.toExport),
		Adapt.array(Adapt.value((a, b) => a.name === b.name)),
	),
});
...
>(
	{
		...
		shouldBeRemovedOnSend: Adapt.v(
			(m) => m.shouldBeRemovedOnSend,
			objectsEqual,
		),
		agent: Adapt.v(
			(m) => m.response?.agent,
		...
		editedFileEvents: Adapt.t(
			(m) => m.editedFileEvents,
			Adapt.array(agentEditedFileEventSchema),
		),
		...
		slashCommand: Adapt.t(
			(m) => m.response?.slashCommand,
			Adapt.value((a, b) => a?.name === b?.name),
		),
		usedContext: Adapt.v((m) => m.response?.usedContext, objectsEqual),
		contentReferences: Adapt.v(
			(m) => m.response?.contentReferences,
			objectsEqual,
		),
	...
	{
		sealed: (o) =>
			o.modelState?.value === ResponseModelState.Cancelled ||
			o.modelState?.value === ResponseModelState.Failed ||
			o.modelState?.value === ResponseModelState.Complete,
	},
);
...
>({
	attachments: Adapt.v(
		(i) => i.attachments.map(IChatRequestVariableEntry.toExport),
		objectsEqual,
	),
	mode: Adapt.v(
		(i) => i.mode,
...
>({
	...
	sendOptions: Adapt.v(
		(p) => serializeSendOptions(p.sendOptions),
		objectsEqual,
	),
});
...
export const storageSchema = Adapt.object<IChatModel, ISerializableChatData>({
	...
	hasPendingEdits: Adapt.v((m) =>
		m.editingSession?.entries
			.get()
			.some((e) => e.state.get() === ModifiedFileEntryState.Modified),
	),
	repoData: Adapt.v((m) => m.repoData, objectsEqual),
	pendingRequests: Adapt.t(
		(m) => m.getPendingRequests(),
		Adapt.array(pendingRequestSchema),
	),
});
