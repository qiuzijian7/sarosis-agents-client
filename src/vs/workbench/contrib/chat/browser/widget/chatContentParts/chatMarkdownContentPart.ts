...
import {
	IMarkdownDiffBlockData,
	MarkdownDiffBlockPart,
	parseUnifiedDiff,
} from "./chatDiffBlockPart.js";
...
export class ChatMarkdownContentPart
	...
	private readonly _onDidChangeDiff = this._register(
		new Emitter<IEditSessionDiffStats>(),
	);
	/**
	...
	readonly onDidChangeDiff: Event<IEditSessionDiffStats> =
		this._onDidChangeDiff.event;

	private readonly allRefs: IDisposableReference<
		CodeBlockPart | CollapsedCodeBlock | MarkdownDiffBlockPart
	>[] = [];
	...
	constructor(
		...
		private readonly aiEditTelemetryService: IAiEditTelemetryService,
	) {
		super();
		...
		if (this.rendererOptions.accessibilityOptions?.statusMessage) {
			this.domNode.ariaLabel =
				this.rendererOptions.accessibilityOptions.statusMessage;
			if (
		...
		const enableMath = configurationService.getValue<boolean>(
		...
		const incrementalRenderingEnabled = configurationService.getValue<boolean>(
			ChatConfiguration.IncrementalRendering,
		);
		if (
			incrementalRenderingEnabled &&
			isResponseVM(element) &&
			fillInIncompleteTokens &&
			!element.isComplete
		) {
			this._incrementalMorpher = this._register(
		...
		const renderStore = this._register(
		...
		const doRenderMarkdown = () => {
					...
					this.markdown.content,
					{
						...
						codeBlockRendererSync: (languageId, text, raw) => {
							...
							if (
								languageId === "diff" &&
								raw &&
								this.rendererOptions.allowInlineDiffs
							) {
								...
								if (match && isResponseVM(context.element)) {
									...
									const { before, after } = parseUnifiedDiff(
										codeBlockUri?.textWithoutResult ?? text,
									);
							...
							if (languageId === "vscode-extensions") {
								const chatExtensions = store.add(
									instantiationService.createInstance(
										ChatExtensionsContentPart,
										{ kind: "extensions", extensions: text.split(",") },
									),
								);
			...
			if (
				isResponseVM(element) &&
				!element.model.codeBlockInfos &&
				element.model.isComplete
			) {
				element.model.initializeCodeBlockInfos(
					this._codeblocks.map((info) => {
						return {
			...
			for (const katexBlock of this.domNode.querySelectorAll(
				...
				const scrollable = new DomScrollableElement(
					katexBlock.cloneNode(true) as HTMLElement,
					{
	...
	private renderCodeBlockPill(
		...
		this.allRefs.push(ref);
		diffListenerStore.add(
			codeBlock.onDidChangeDiff(() => this.fireAggregatedDiff()),
		);
	...
	hasSameContent(other: IChatProgressRenderableResponseContent): boolean {
		...
		if (
			lastCodeblock &&
			lastCodeblock.codemapperUri !== undefined &&
			lastCodeblock.isStreamingEdit
		) {
			return (
	...
	tryIncrementalUpdate(newMarkdown: IChatMarkdownContent): boolean {
		...
		const success = this._incrementalMorpher.tryMorph(
			newMarkdown.content.value,
		);
		...
		this.allRefs.forEach((ref, index) => {
			if (ref.object instanceof CodeBlockPart) {
				...
				if (
					...
					!isEqual(ref.object.uri, codeblockModel.codemapperUri)
				) {
...
export class CollapsedCodeBlock extends Disposable {
	...
	private readonly _onDidChangeDiff = this._register(
		new Emitter<IEditSessionEntryDiff>(),
	);
	readonly onDidChangeDiff: Event<IEditSessionEntryDiff> =
		this._onDidChangeDiff.event;
	...
	private registerListeners(): void {
		this._register(
			registerOpenEditorListeners(this.pillElement, (e) => this.showDiff(e)),
		);
	...
	render(uri: URI): void {
		...
		this.pillElement.replaceChildren(
			progressFill,
			iconEl,
			iconLabelEl,
			labelDetail,
		...
		const isStreaming = derived((r) => {
			...
			return (
				!!currentlyModified &&
				currentlyModified.responseModel.requestId === this.requestId &&
				currentlyModified.undoStopId === this.inUndoStop
			);
		});
		...
		this.progressStore.add(
			autorun((r) => {
				...
				if (isStreaming.read(r)) {
					...
					const showAnimation = this.configurationService.getValue<boolean>(
					...
					const fileKind = uri.path.endsWith("/")
						? FileKind.FOLDER
						: FileKind.FILE;
					pillIconClasses = getIconClasses(
						this.modelService,
