...
} from "../../common/widget/annotations.js";
...
import {
	DiffEditorPool,
	EditorPool,
} from "./chatContentParts/chatContentCodePools.js";
import {
	IChatContentPart,
...
import {
	...
	CodeBlockPart,
} from "./chatContentParts/codeBlockPart.js";
import {
	autorun,
	observableValue,
...
function upvoteAnimationSettingToEnum(
...
export class ChatListItemRenderer
	...
	private readonly templateDataByRequestId = new Map<
		string,
		IChatListItemTemplate
	>();
	private readonly responseTemplateDataByRequestId = new Map<
		string,
		IChatListItemTemplate
	>();

	/** Track pending question carousels by session resource for auto-skip on chat submission */
	private readonly pendingQuestionCarousels = new ResourceMap<
		Set<ChatQuestionCarouselPart>
	>();
	private readonly _notifiedQuestionCarousels = new Set<string>();
	private readonly workingProgressConfirmationEndListeners =
		new WeakSet<IChatToolInvocation>();
	...
	private readonly _onDidClickRerunWithAgentOrCommandDetection = this._register(
		new Emitter<{
			readonly sessionResource: URI;
			readonly requestId: string;
		}>(),
	);
	readonly onDidClickRerunWithAgentOrCommandDetection =
		this._onDidClickRerunWithAgentOrCommandDetection.event;

	private readonly _onDidClickRequest = this._register(
		new Emitter<IChatListItemTemplate>(),
	);
	readonly onDidClickRequest: Event<IChatListItemTemplate> =
		this._onDidClickRequest.event;

	private readonly _onDidRerender = this._register(
		new Emitter<IChatListItemTemplate>(),
	);
	readonly onDidRerender: Event<IChatListItemTemplate> =
		this._onDidRerender.event;
	...
	constructor(
		...
		this.chatContentMarkdownRenderer = this.instantiationService.createInstance(
		...
		this._inlineTextModels = this._register(
			this.instantiationService.createInstance(InlineTextModelCollection),
		);
		this._register(
			this.instantiationService.createInstance(ChatCodeBlockContentProvider),
		);
		// Auto-skip pending question carousels when user submits a new chat message
		this._register(
			this.chatService.onDidSubmitRequest((e) => {
				const carousels = this.pendingQuestionCarousels.get(
		...
		this._register(
			this.configService.onDidChangeConfiguration((e) => {
				if (
					e.affectsConfiguration(ChatConfiguration.AutoReply) &&
					this.configService.getValue<boolean>(ChatConfiguration.AutoReply)
				) {
	...
	private fireItemHeightChange(
		template: IChatListItemTemplate,
		...
		const height =
			measuredHeight ?? template.rowContainer.getBoundingClientRect().height;
		if (height === 0 || !height) {
			return;
		}
	...
	renderTemplate(container: HTMLElement): IChatListItemTemplate {
		...
		const disabledOverlay = dom.append(
			container,
		...
		const checkpointContainer = dom.append(
			rowContainer,
				...
				MenuId.ChatMessageFooter,
				{
					...
					actionViewItemProvider: (
						...
						if (
							...
							action.item.id === MarkHelpfulActionId
						) {
							const animation = upvoteAnimationSettingToEnum(
								this.configService.getValue<string>("chat.upvoteAnimation"),
							);
							return scopedInstantiationService.createInstance(
								MenuEntryActionViewItem,
								action,
								{ ...options, onClickAnimation: animation },
							);
						}
		...
		const footerDetailsContainer = dom.append(
			footerToolbar.getElement(),
			$(".chat-footer-details"),
		);
		footerDetailsContainer.tabIndex = 0;

		const checkpointRestoreContainer = dom.append(
			rowContainer,
		...
		const label = dom.append(
			...
			$("span.checkpoint-label-text"),
		);
		label.textContent = localize("checkpointRestore", "Checkpoint Restored");
		const dot = dom.append(
			checkpointRestoreContainer,
			$("span.checkpoint-dot-separator"),
		);
		dot.textContent = "\u00B7";
		dot.setAttribute("aria-hidden", "true");
		const checkpointRestoreToolbar = templateDisposables.add(
			scopedInstantiationService.createInstance(
				MenuWorkbenchToolBar,
				checkpointRestoreContainer,
		...
		const connectionObserver = document.createElement(
			"connection-observer",
		) as dom.ConnectionObserverElement;
		...
		templateDisposables.add(
			this._onDidUpdateViewModel.event(() => {
				if (
					!template.currentElement ||
					!this.viewModel?.sessionResource ||
					!isEqual(
						template.currentElement.sessionResource,
						this.viewModel.sessionResource,
					)
				) {
		...
		templateDisposables.add(
			dom.addDisposableListener(disabledOverlay, dom.EventType.CLICK, (e) => {
		...
		const resizeObserver = templateDisposables.add(
			new dom.DisposableResizeObserver(
				"ChatListItemRenderer.itemHeight",
				(entries) => {
					const entry = entries[0];
					if (entry) {
						this.fireItemHeightChange(
							template,
							entry.borderBoxSize.at(0)?.blockSize,
						);
					}
				},
			),
	...
	private renderChatTreeItem(
		...
		if (
			...
			templateData.currentElement.id !== element.id
		) {
			this.traceLayout(
				"renderChatTreeItem",
				`Rendering a different element into the template, index=${index}`,
			);
			const mappedTemplateData = this.templateDataByRequestId.get(
				templateData.currentElement.id,
			);
			if (
				mappedTemplateData &&
				mappedTemplateData.currentElement?.id !== templateData.currentElement.id
			) {
		...
		ChatContextKeys.isFirstRequest
			.bindTo(templateData.contextKeyService)
			.set(
				isRequestVM(element) &&
		...
		ChatContextKeys.isPendingRequest
			.bindTo(templateData.contextKeyService)
			.set(isRequestVM(element) && !!element.pendingKind);
		...
		const location = this.chatWidgetService.getWidgetBySessionResource(
		...
		const updateContainerCheckmarks = () =>
			templateData.rowContainer.classList.toggle(
				"show-checkmarks",
				!!this.configService.getValue<boolean>(
		...
		const isSystemInitiatedRequest =
			isRequestVM(element) && !!element.isSystemInitiated;
		...
		const supportsForkOrRestoration =
			this.rendererOptions.supportsFork ||
		...
		if (isResponseVM(element)) {
			...
			templateData.elementDisposables.add(
				toDisposable(() =>
					this.responseTemplateDataByRequestId.delete(element.requestId),
				),
			);
		}
		...
		templateData.requestHover.classList.toggle(
			"expanded",
			this.configService.getValue<string>("chat.editRequests") === "hover",
		);
		templateData.requestHover.classList.toggle(
			"checkpoints-enabled",
			checkpointEnabled,
		...
		const incrementalRendering = this.configService.getValue<boolean>(
	...
	private renderPendingDivider(
		element: IChatPendingDividerViewModel,
		templateData: IChatListItemTemplate,
	): void {
		...
		const label = dom.append(
			dividerContent,
			dom.$("span.pending-divider-label"),
		);
	...
	private renderConfirmationAction(
		...
		if (element.confirmation) {
			dom.append(
				templateData.detail,
				$("span.codicon.codicon-check", { "aria-hidden": "true" }),
			);
			dom.append(
				templateData.detail,
				$(
	...
	private getAgentIcon(agent: IChatAgentMetadata | undefined): URI | ThemeIcon {
		if (agent?.themeIcon) {
			...
			agent?.iconDark &&
			isDark(this.themeService.getColorTheme().type)
		) {
	...
	private renderChatResponseBasic(
		...
		if (
			element.model.response === element.model.entireResponse &&
			element.errorDetails?.message &&
			element.errorDetails.message !== canceledName
		) {
			content.push({
				kind: "errorDetails",
		...
		const workingProgress = this.shouldShowWorkingProgress(
			element,
			content,
			false,
			templateData,
	...
	private shouldShowWorkingProgress(
		element: IChatResponseViewModel,
		partsToRender: IChatRendererContent[],
		moreContentAvailable: boolean,
		templateData: IChatListItemTemplate,
	): IChatWorkingProgress | undefined {
		if (
		...
		if (
			partsToRender.some((part) => part.kind === "planReview" && !part.isUsed)
		) {
			return undefined;
		}
		...
		if (isResponseVM(element)) {
			...
			if (widget?.inputPart.hasActiveToolConfirmationCarousel) {
				const nonSubagentConfirmationCount =
					this.getPendingToolConfirmationCount(partsToRender, false);
				if (nonSubagentConfirmationCount > 0) {
					return {
						kind: "working",
						content: new MarkdownString().appendText(
							this.getConfirmationPendingLabel(nonSubagentConfirmationCount),
						),
					};
				}

				if (
					this.getPendingToolConfirmationCount(partsToRender, true) > 0 ||
					this.getSubagentPart(templateData.renderedParts)
				) {
					return undefined;
				}
				...
				return {
					kind: "working",
					content: new MarkdownString().appendText(
						this.getConfirmationPendingLabel(1),
					),
				};
			}
		}
		...
		for (let i = partsToRender.length - 1; i >= 0; i--) {
			...
			if (
				...
				part.content.value.trim().length > 0
			) {
				lastPart = part;
				break;
			}
		...
		if (
			...
			(lastPart.kind === "toolInvocation" ||
				lastPart.kind === "toolInvocationSerialized")
		) {
			if (lastPart.isAttachedToThinking) {
				return undefined;
			}

			const isEffectivelyHiddenToolInvocation =
				IChatToolInvocation.isEffectivelyHidden(lastPart);
			const collapsedToolsMode =
				this.configService.getValue<CollapsedToolsDisplayMode>(
					"chat.agent.thinking.collapsedTools",
				);
			if (
				!isEffectivelyHiddenToolInvocation &&
				collapsedToolsMode !== CollapsedToolsDisplayMode.Off &&
				this.shouldPinPart(
					lastPart,
					isResponseVM(element) ? element : undefined,
				)
			) {
				return undefined;
			}
		...
		const hasRenderedThinkingPart = (templateData.renderedParts ?? []).some(
			(part) => part instanceof ChatThinkingContentPart,
		);
		const hasEditPillMarkdown = partsToRender.some(
			(part) =>
				part.kind === "markdownContent" && this.hasEditCodeblockUri(part),
		);
		...
		if (
			...
			(lastPart.kind === "markdownContent" &&
				...
				this.hasBeenCaughtUpLongEnough(element)) ||
			((lastPart.kind === "toolInvocation" ||
				lastPart.kind === "toolInvocationSerialized") &&
				(IChatToolInvocation.isComplete(lastPart) ||
					IChatToolInvocation.isEffectivelyHidden(lastPart))) ||
			((lastPart.kind === "textEditGroup" ||
				lastPart.kind === "notebookEditGroup") &&
				lastPart.done &&
				!partsToRender.some(
					(part) =>
						part.kind === "toolInvocation" &&
	...
	private getPendingToolConfirmationCount(
		parts: ReadonlyArray<IChatRendererContent | IChatProgressResponseContent>,
		includeSubagentConfirmations: boolean,
	): number {
	...
	private updateWorkingProgressForPendingConfirmations(
		...
		const pendingConfirmationCount = this.getPendingToolConfirmationCount(
		...
		const workingProgressPart =
			this.getWorkingProgressContentPart(templateData);
		if (workingProgressPart) {
			workingProgressPart.updateWorkingContent(
				new MarkdownString().appendText(
					this.getConfirmationPendingLabel(pendingConfirmationCount),
				),
			);
		}
	}
	...
	private getWorkingProgressContentPart(
		templateData: IChatListItemTemplate,
	): ChatWorkingProgressContentPart | undefined {
		const renderedParts = templateData.renderedParts;
		if (!renderedParts) {
			return undefined;
		}
	...
	private getChatFileChangesSummaryPart(
		element: IChatResponseViewModel,
	): IChatChangesSummaryPart | undefined {
		if (!this.shouldShowFileChangesSummary(element)) {
			return undefined;
		}
		if (
			!element.model.entireResponse.value.some(
				(part) =>
	...
	private renderChatRequest(
		...
		if (element.pendingKind && this._pendingDragController) {
			...
			const sameKindCount = (
				this.viewModel?.model.getPendingRequests() ?? []
			).filter((p) => p.kind === element.pendingKind).length;
			if (sameKindCount > 1) {
				const handle = dom.$(
		...
		if (
			this.configService.getValue<string>("chat.editRequests") !== "none" &&
			this.rendererOptions.editable
		) {
			templateData.elementDisposables.add(
				dom.addDisposableListener(
	...
	private renderSystemInitiatedRequest(
		...
		const rendered = this.chatContentMarkdownRenderer.render(
	...
	private doIncrementalRender(
		...
		const contentForThisTurn = this.getNextProgressiveRenderContent(
			element,
			templateData,
		);
	...
	private logIncrementalRenderingTelemetry(): void {
		...
		type IncrementalRenderingSettingsClassification = {
			animationStyle: {
				...
				comment: "The animation style selected for incremental rendering.";
			};
			buffering: {
				classification: "SystemMetaData";
				purpose: "FeatureInsight";
				comment: "The buffering mode selected for incremental rendering.";
			};
			owner: "pwang347";
			comment: "Tracks which incremental rendering settings are in use.";
		};
		this.telemetryService.publicLog2<
			IncrementalRenderingSettingsEvent,
			IncrementalRenderingSettingsClassification
		>("chatIncrementalRenderingSettings", {
			animationStyle:
	...
	private renderChatContentDiff(
		...
		partsToRender.forEach((partToRender, contentIndex) => {
			...
			if (alreadyRenderedPart) {
				if (
					partToRender.kind === "thinking" &&
					alreadyRenderedPart instanceof ChatThinkingContentPart
				) {
				...
				if (
					partToRender.kind === "markdownContent" &&
				...
				if (alreadyRenderedPart.domNode) {
					const thinkingToolWrapper = dom.findParentWithClass(
						alreadyRenderedPart.domNode,
			...
			if (
				lastThinking &&
				(partToRender.kind === "toolInvocation" ||
					partToRender.kind === "toolInvocationSerialized" ||
					partToRender.kind === "markdownContent" ||
					partToRender.kind === "textEditGroup" ||
					partToRender.kind === "hook") &&
				this.shouldPinPart(partToRender, element)
			) {
				if (alreadyRenderedPart instanceof ChatMarkdownContentPart) {
					lastThinking.removeEditPillByPartId(
						alreadyRenderedPart.codeblocksPartId,
					);
				}
	...
	private getNextProgressiveRenderContent(
		element: IChatResponseViewModel,
		templateData: IChatListItemTemplate,
	): { content: IChatRendererContent[]; moreContentAvailable: boolean } {
			...
			if (
				...
				if (numNeededWords <= 0) {
					...
					if (
						renderableResponse
							.slice(i + 1)
							.some((part) => part.kind === "markdownContent")
					) {
						moreContentAvailable = true;
					}
					break;
	...
	private shouldShowFileChangesSummary(
		...
		const isLocalSession =
			sessionType === localChatSessionType || isAgentHostTarget(sessionType);
		return (
			element.isComplete &&
			isLocalSession &&
	...
	private getDataForProgressiveRender(element: IChatResponseViewModel) {
		const hasMarkdownParts = element.response.value.some(
			(part) =>
				part.kind === "markdownContent" && part.content.value.trim().length > 0,
		);
		if (
			!element.isComplete &&
			hasMarkdownParts &&
			(element.contentUpdateTimings
				? element.contentUpdateTimings.lastWordCount
				: 0) === 0
		) {
			/**
	...
	private isCodeblockComplete(
		...
		return (
			!isResponseVM(element) ||
			element.isComplete ||
			codeblockHasClosingBackticks(part.content.value)
		);
	}
	...
	private shouldPinPart(
		...
		if (part.kind === "hook") {
			...
			return (
				part.hookType === HookType.PreToolUse ||
				part.hookType === HookType.PostToolUse
			);
		}
		...
		const isMermaidTool =
			(part.kind === "toolInvocation" ||
				part.kind === "toolInvocationSerialized") &&
			part.toolId.toLowerCase().includes("mermaid");
		if (isMermaidTool) {
			return false;
		}

		// don't pin ask questions tool invocations
		const isAskQuestionsTool =
			(part.kind === "toolInvocation" ||
				part.kind === "toolInvocationSerialized") &&
			(part.toolId === "copilot_askQuestions" ||
				part.toolId === "vscode_askQuestions");
		...
		const isTerminalTool =
			(part.kind === "toolInvocation" ||
				part.kind === "toolInvocationSerialized") &&
			part.toolSpecificData?.kind === "terminal";
		const isContributedTerminalToolInvocation =
			element &&
			element.sessionResource.scheme !== Schemas.vscodeChatInput &&
			getChatSessionType(element.sessionResource) !== localChatSessionType && // contributed sessions
			part.kind === "toolInvocationSerialized" &&
			part.toolSpecificData?.kind === "terminal"; // contributed serialized terminal tool invocations data
		if (isTerminalTool && !isContributedTerminalToolInvocation) {
			// don't pin terminals with confirmation
			if (
				part.kind === "toolInvocation" &&
				IChatToolInvocation.getConfirmationMessages(part)
			) {
				return false;
			}
			const terminalToolsInThinking = this.configService.getValue<boolean>(
	...
	private getLastThinkingPart(
		renderedParts: ReadonlyArray<IChatContentPart> | undefined,
	): ChatThinkingContentPart | undefined {
	...
	): ChatSubagentContentPart | undefined {
		if (!renderedParts || renderedParts.length === 0) {
			return undefined;
		}
	...
	private finalizeAllSubagentParts(templateData: IChatListItemTemplate): void {
		...
		for (const part of templateData.renderedParts) {
			if (
				part instanceof ChatSubagentContentPart &&
				part.getIsActive() &&
	...
	private handleSubagentToolGrouping(
		...
		const lastSubagent = this.getSubagentPart(
			templateData.renderedParts,
			subagentId,
		);
		if (lastSubagent) {
			// Enable carousel mode before appendToolInvocation creates an inline part.
			this.maybeRouteSubagentToolToCarousel(
				toolInvocation,
				lastSubagent,
				context,
				templateData,
	...
	private maybeRouteSubagentToolToCarousel(
		...
		if (
			isParentSubagentTool(toolInvocation) ||
		...
		const scrollToSubagent = (targetSubAgentId: string) => {
			const currentTemplateData = this.getTemplateDataForRequestId(
				context.element.id,
			);
			const currentSubagentPart =
				this.getSubagentPart(
					currentTemplateData?.renderedParts,
					targetSubAgentId,
				) ?? subagentPart;
		...
		const factory = (tool: IChatToolInvocation) =>
			this.instantiationService.createInstance(
		...
		const addToolToCarousel = (tool: IChatToolInvocation) => {
			...
			const listener = this.createUpdateWorkingProgressOnConfirmationEnd(
				tool,
				templateData,
			);
			if (listener) {
		...
		const shouldUseCarouselForTool = (
		...
		subagentPart.enableCarouselMode(
			navigateToCarousel,
			addToolToCarousel,
			shouldUseCarouselForTool,
		);
	...
	private finalizeCurrentThinkingPart(
		...
		const style = this.configService.getValue<ThinkingDisplayMode>(
	...
	private renderChatContentPart(
		...
		try {
			...
			if (
				...
				const lastThinking = this.getLastThinkingPart(
					templateData.renderedParts,
				);
				lastThinking?.resetId();
			...
			const shouldPin = this.shouldPinPart(
				content,
				isResponseElement ? context.element : undefined,
			);
			...
			if (context.element.isComplete && !shouldPin) {
				const elementTemplateData = this.getTemplateDataForRequestId(
					context.element.id,
				);
				if (elementTemplateData?.renderedParts) {
					const lastThinking = this.getLastThinkingPart(
						elementTemplateData.renderedParts,
					);
			...
			const isSubagentContent =
				(content.kind === "toolInvocation" ||
					content.kind === "toolInvocationSerialized") &&
			...
			if (content.kind === "treeData") {
				...
				content.kind === "progressTask" ||
				content.kind === "progressTaskSerialized"
			) {
				...
				if (
					isResponseVM(context.element) &&
					context.element.agent?.isDefault &&
					!context.element.agent.modes.includes(ChatModeKind.Ask)
				) {
				...
				content.kind === "elicitationSerialized"
			) {
				...
				return this.renderMcpServersInteractionRequired(
					content,
					context,
					templateData,
				);
			...
			} else if (content.kind === "step") {
				// Render step start/end as markdown content
				const stepText =
					content.phase === "start"
						? `Started: ${content.stepName}`
						: `Finished: ${content.stepName}`;
				return this.renderMarkdown(
					{
						kind: "markdownContent",
						content: new MarkdownString(`**${stepText}**`),
					},
					templateData,
					context,
				);
			} else if (content.kind === "runError") {
				// Render runError as error content
				const level =
					content.level === "error"
				...
				return this.instantiationService.createInstance(
					ChatErrorContentPart,
					level,
					new MarkdownString(content.message),
					content,
					this.chatContentMarkdownRenderer,
				);
			}
			...
			const errorPart = this.instantiationService.createInstance(
				ChatErrorContentPart,
				ChatErrorLevel.Error,
				new MarkdownString(
	...
	private renderChatErrorDetails(
		...
		if (content.errorDetails.isQuotaExceeded) {
			...
			content.errorDetails.isRateLimited &&
			...
			const errorConfirmation = this.instantiationService.createInstance(
				ChatErrorConfirmationContentPart,
				level,
				new MarkdownString(content.errorDetails.message),
				content,
				content.errorDetails.confirmationButtons,
				this.chatContentMarkdownRenderer,
				context,
			);
		...
		if (isResponseVM(context.element)) {
			...
			treePart.addDisposable(
				toDisposable(() =>
					this.fileTreesByResponseId.set(
						context.element.id,
						fileTrees.filter((v) => v.treeDataId !== data.uri.toString()),
					),
				),
			);
		}
	...
	private renderMultiDiffData(
		content: IChatMultiDiffData | IChatMultiDiffDataSerialized,
		templateData: IChatListItemTemplate,
		context: IChatContentPartRenderContext,
	): IChatContentPart {
		const multiDiffPart = this.instantiationService.createInstance(
	...
	private renderToolInvocation(
		...
		const createToolPart = (): {
			domNode: HTMLElement;
			disposable: ChatToolInvocationPart;
		...
		if (
			...
			return this.handleSubagentToolGrouping(
				toolInvocation,
				subagentId,
				context,
				templateData,
				...
				part.addDisposable(
					autorun((reader) => {
						...
						if (isCarouselConfirmation) {
							...
							IChatToolInvocation.isEffectivelyHidden(toolInvocation, reader)
						) {
							this.updateWorkingProgressForPendingConfirmations(templateData);
							dom.hide(part.domNode);
						} else {
	...
	private setupConfirmationTransitionWatcher(
		...
		if (
			currentState.type === IChatToolInvocation.StateKind.WaitingForConfirmation
		) {
		...
		const disposable = autorun((reader) => {
			...
			if (
				...
				if (
					state.type !== IChatToolInvocation.StateKind.WaitingForConfirmation ||
					!tryRouteConfirmationToCarousel()
				) {
					moveConfirmationWidgetOutOfThinking();
				}
			}
	...
	private renderExtensionsContent(
		extensionsContent: IChatExtensionsContent,
		context: IChatContentPartRenderContext,
		templateData: IChatListItemTemplate,
	): IChatContentPart | undefined {
	...
	private renderHookPart(
		...
		if (!(hookPart.stopReason || hookPart.systemMessage)) {
			return this.renderNoContent(
				(other) =>
					other.kind === "hook" && other.hookType === hookPart.hookType,
			);
		}
		...
		if (hookPart.subAgentInvocationId) {
			...
			if (subagentPart) {
				...
				return this.renderNoContent(
					(other) =>
						other.kind === "hook" &&
						other.hookType === hookPart.hookType &&
						other.subAgentInvocationId === hookPart.subAgentInvocationId,
				);
			}
		}

		// Only pin preTool/postTool hooks into the thinking part
		const shouldPinToThinking =
			hookPart.hookType === HookType.PreToolUse ||
			hookPart.hookType === HookType.PostToolUse;
	...
	private renderPullRequestContent(
		pullRequestContent: IChatPullRequestContent,
		context: IChatContentPartRenderContext,
		templateData: IChatListItemTemplate,
	): IChatContentPart | undefined {
		const part = this.instantiationService.createInstance(
	...
	private renderProgressTask(
		task: IChatTask | IChatTaskSerialized,
		templateData: IChatListItemTemplate,
		context: IChatContentPartRenderContext,
	): IChatContentPart | undefined {
		if (!isResponseVM(context.element)) {
	...
	private renderElicitation(
		context: IChatContentPartRenderContext,
		elicitation: IChatElicitationRequest | IChatElicitationRequestSerialized,
		templateData: IChatListItemTemplate,
	): IChatContentPart {
		if (
			elicitation.kind === "elicitationSerialized"
				? elicitation.isHidden
				: elicitation.isHidden?.get()
		) {
	...
	private renderQuestionCarousel(
		...
		if (!carousel.terminalId && isResponseVM(context.element)) {
			...
			const model = this.chatService.getSession(
				responseElement.sessionResource,
			);
			const request = model
				?.getRequests()
				.find((r) => r.id === responseElement.requestId);
		...
		const responseId = isResponseVM(context.element)
			...
			: undefined;
		const carouselKey =
			carousel.resolveId ?? `${responseId ?? ""}_${context.contentIndex}`;
		...
		const handleSubmit = async (
			...
			const answersRecord: IChatQuestionAnswers | undefined = answers
		...
		if (carousel.isUsed || responseIsComplete) {
			if (
				...
				this.pendingQuestionCarousels
					.get(context.element.sessionResource)
					?.clear();
			}
		...
		if (!part) {
			const fallbackPart = this.instantiationService.createInstance(
				ChatQuestionCarouselPart,
				carousel,
				context,
				{
		...
		if (
			...
			let carousels = this.pendingQuestionCarousels.get(
				context.element.sessionResource,
			);
			if (!carousels) {
				carousels = new Set();
				this.pendingQuestionCarousels.set(
					context.element.sessionResource,
					carousels,
				);
			}
	...
	private _getCarouselStableKey(
		...
		const requestId = isResponseVM(context.element)
			? context.element.requestId
			: undefined;
	...
	private _notifyOnQuestionCarousel(
		...
		const question =
			carousel.questions.length > 0 && carousel.questions[0].message
				? carousel.questions[0].message
				: localize("chat.questionCarouselNeedsInputSR", "Chat input required.");
		const stringQuestion =
			typeof question === "string" ? question : question.value;
		const alertMessage =
			questionCount === 1
				? localize(
	...
	private renderPlanReview(
		...
		const responseId = isResponseVM(context.element)
			...
			: undefined;
		const reviewKey =
			review.resolveId ?? `${responseId ?? ""}_${context.contentIndex}`;
		...
		const renderProgress = (): IChatContentPart => {
			...
			const data =
				renderedAsUsed && !review.data?.rejected ? review.data : undefined;
	...
	private renderMarkdown(
		...
		if (isRequestVM(element)) {
			...
			if (
				this.configService.getValue<string>("chat.editRequests") === "inline" &&
				this.rendererOptions.editable
			) {
				...
				markdownPart.addDisposable(
					dom.addDisposableListener(
						...
						(e: MouseEvent) => {
							...
							const selection = dom
								.getWindow(templateData.rowContainer)
								.getSelection();
							if (
								selection &&
							...
							const monacoEditor = dom.findParentWithClass(
								clickedElement,
								"monaco-editor",
							);
							if (monacoEditor) {
								const editorPart = Array.from(this.editorsInUse()).find(
									(editor) => editor.element.contains(monacoEditor),
								);
		...
		if (
			...
			const subAgentInvocationId = extractSubAgentInvocationIdFromText(
			...
			if (
				!lastThinking &&
				markdownPart?.domNode &&
				this.shouldPinPart(markdown, context.element) &&
				collapsedToolsMode === CollapsedToolsDisplayMode.Always &&
				isComplete
			) {
				const thinkingPart = this.renderThinkingPart(
	...
	renderThinkingPart(
		...
		const streamingCompleted = this.isThinkingLookAheadComplete(
			context,
			element,
		);
		...
		if (Array.isArray(content.value)) {
			...
			return (
				lastPart ?? this.renderNoContent((other) => content.kind === other.kind)
			);
			// non-array, handle case where we are currently thinking vs. starting a new thinking part
		} else {
			const lastActiveThinking = this.getLastThinkingPart(
				templateData.renderedParts,
			);
	...
	disposeElement(
		...
		if (
			isRequestVM(node.element) &&
			node.element.id === this.viewModel?.editing?.id &&
			details?.onScroll
		) {
	...
	private renderMcpServersInteractionRequired(
		content: IChatMcpServersStarting | IChatMcpServersStartingSerialized,
		context: IChatContentPartRenderContext,
		templateData: IChatListItemTemplate,
	): IChatContentPart {
		return this.instantiationService.createInstance(
	...
	private renderDisabledClaudeHooks(
		content: IChatDisabledClaudeHooksPart,
		context: IChatContentPartRenderContext,
	): IChatContentPart {
		return this.instantiationService.createInstance(
 ...
 */
function isSubagentToolInvocation(
	invocation: IChatToolInvocation | IChatToolInvocationSerialized,
): boolean {
