...
} from "../../../../platform/networkFilter/common/networkFilterService.js";
...
import {
	...
	AssistedTypes,
} from "../../mcp/browser/mcpCommandsAddConfiguration.js";
import {
	allDiscoverySources,
...
import {
	CodeMapperService,
	ICodeMapperService,
} from "../common/editing/chatCodeMapperService.js";
...
} from "../common/widget/chatResponseResourceFileSystemProvider.js";
...
} from "../common/participants/chatSlashCommands.js";
import {
	ChatArtifactsService,
	IChatArtifactsService,
} from "../common/tools/chatArtifactsService.js";
import {
	ChatTodoListService,
	IChatTodoListService,
} from "../common/tools/chatTodoListService.js";
import {
	ChatTransferService,
	IChatTransferService,
} from "../common/model/chatTransferService.js";
import { IChatVariablesService } from "../common/attachments/chatVariables.js";
import {
	ChatWidgetHistoryService,
	IChatWidgetHistoryService,
} from "../common/widget/chatWidgetHistoryService.js";
import {
	ChatAgentLocation,
	ChatConfiguration,
...
} from "./tools/chatToolRiskAssessmentService.js";
import {
...
import {
	isTildePath,
	PromptsConfig,
} from "../common/promptSyntax/config/config.js";
import {
...
} from "./actions/chatCopyActions.js";
...
} from "./chatDebug/chatDebugEditorInput.js";
...
} from "./attachments/chatAttachmentResolveService.js";
import {
	ChatAttachmentWidgetRegistry,
	IChatAttachmentWidgetRegistry,
} from "./attachments/chatAttachmentWidgetRegistry.js";
import {
	ChatMarkdownAnchorService,
	IChatMarkdownAnchorService,
} from "./widget/chatContentParts/chatMarkdownAnchorService.js";
import {
...
import {
	ChatEditor,
	IChatEditorOptions,
} from "./widgetHosts/editor/chatEditor.js";
import {
	ChatEditorInput,
	ChatEditorInputSerializer,
} from "./widgetHosts/editor/chatEditorInput.js";
...
} from "./languageModelsConfigurationService.js";
...
} from "./chatOutputItemRenderer.js";
import {
...
} from "../common/plugins/agentPluginServiceImpl.js";
...
} from "../common/plugins/workspacePluginSettingsService.js";
...
} from "./planReviewFeedback/planReviewFeedbackService.js";
import { PluginUrlHandler } from "./pluginUrlHandler.js";
import { PromptUrlHandler } from "./promptSyntax/promptUrlHandler.js";
import {
	ConfigureToolSets,
	UserToolSetsContributions,
} from "./tools/toolSetsContribution.js";
...
} from "./chatImageCarouselService.js";
...
CommandsRegistry.registerCommand(
	...
	(
		...
		resolveId: string,
		answers?: import("../common/chatService/chatService.js").IChatQuestionAnswers,
	) => {
		accessor
			.get(IChatService)
			.notifyQuestionCarouselAnswer("", resolveId, answers);
	},
);
...
const jsonContributionRegistry = Registry.as<IJSONContributionRegistry>(
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		"chat.experimental.renderMarkdownImmediately": {
			...
			description: nls.localize(
				"chat.experimental.renderMarkdownImmediately",
				"When enabled, markdown content is rendered immediately as it arrives, instead of waiting for all content to be received before rendering the formatted markdown.",
			),
			default: true,
			tags: ["experimental"],
		},
		[ChatConfiguration.SymbolToolsCacheStable]: {
			...
			description: nls.localize(
				...
				"When enabled, the rename and list-code-usages tools are always registered with a static description (no per-language list). Stabilizes the tools-array bytes across requests so prompt caches survive language-extension activations mid-turn. Tool behavior is unchanged: unsupported languages still produce an error at invocation time.",
			),
			default: false,
			tags: ["experimental"],
			experiment: {
		...
		[ChatConfiguration.SessionSyncEnabled]: {
			...
			policy: {
				...
				value: (policyData) =>
					policyData.session_search === CopilotSessionSearchPolicy.Disabled
						? false
						: undefined,
				localization: {
					description: {
			...
			additionalProperties: {
				...
				properties: {
					...
					onlyShowGroup: {
						type: "boolean",
						description: nls.localize(
							"chat.artifacts.rules.byFilePath.onlyShowGroup",
							"When true, show only the group header instead of individual items.",
						),
					},
				},
			...
			additionalProperties: {
				...
				properties: {
					groupName: {
						type: "string",
						description: nls.localize(
							"chat.artifacts.rules.byMemoryFilePath.groupName",
							"Display name for the artifact group.",
						),
					},
					onlyShowGroup: {
						type: "boolean",
						description: nls.localize(
							"chat.artifacts.rules.byMemoryFilePath.onlyShowGroup",
							"When true, show only the group header instead of individual items.",
						),
					},
				},
		...
		[mcpServerSamplingSection]: {
			...
			description: nls.localize(
				...
				"MCP: " + nls.localize("mcp.list", "List Servers"),
			),
		...
		[ChatConfiguration.AgentEnabled]: {
			...
			policy: {
				...
				minimumVersion: "1.99",
				value: (policyData) =>
					policyData.chat_agent_enabled === false ? false : undefined,
				localization: {
					description: {
		...
		[mcpDiscoverySection]: {
			...
			properties: Object.fromEntries(
				allDiscoverySources.map((k) => [
					k,
					{ type: "boolean", description: discoverySourceSettingsLabel[k] },
				]),
			),
		...
		[PromptsConfig.HOOKS_LOCATION_KEY]: {
			...
			default: {
				...DEFAULT_HOOK_FILE_PATHS.map((f) => ({ [f.path]: true })).reduce(
					(acc, curr) => ({ ...acc, ...curr }),
					{},
				),
			},
...
Registry.as<IConfigurationMigrationRegistry>(
	...
	},
	{
		...
		migrateFn: (value, accessor) => {
			...
			if (
				value !== undefined &&
				accessor(AgentNetworkDomainSettingId.DeniedNetworkDomains) === undefined
			) {
				pairs.push([
					AgentNetworkDomainSettingId.DeniedNetworkDomains,
					{ value },
...
class ChatResolverContribution extends Disposable {
	...
	private readonly _editorRegistrations = this._register(
	...
	constructor(
		...
		this._register(
			chatSessionsService.onDidChangeContentProviderSchemes((e) => {
...
class CopilotTelemetryContribution
	...
	private updateCopilotTrackingId(): void {
		...
		if (copilotTrackingId) {
			...
			this.telemetryService.setCommonProperty(
		...
		editorResolverService.registerEditor(
			...
			},
			{
				...
				canSupportResource: (resource) =>
					resource.scheme === ChatDebugEditorInput.RESOURCE.scheme,
			},
			{
				createEditorInput: () => {
					return {
...
class ChatAgentSettingContribution
	...
	constructor(
		...
		@IChatEntitlementService
		private readonly entitlementService: IChatEntitlementService,
		@IContextKeyService private readonly contextKeyService: IContextKeyService,
	) {
		super();
		this.newChatButtonExperimentIcon =
			ChatContextKeys.newChatButtonExperimentIcon.bindTo(
	...
	private registerNewChatButtonIcon(): void {
		this.experimentService
			.getTreatment<string>("chatNewButtonIcon")
			.then((value) => {
	...
	private registerDefaultModeSetting(): void {
		this.experimentService
			.getTreatment<string>("chatDefaultNewSessionMode")
			.then((value) => {
				const node: IConfigurationNode = {
					id: "chatSidebar",
					title: nls.localize("interactiveSessionConfigurationTitle", "Chat"),
...
class ChatForegroundSessionCountContribution
	...
	constructor(
		...
		super();
		this.foregroundSessionCountContextKey =
			ChatContextKeys.foregroundSessionCount.bindTo(this.contextKeyService);

		this._register(
			this.chatWidgetService.onDidAddWidget(() => {
	...
	private updateForegroundSessionCount(): void {
		...
		for (const widget of this.chatWidgetService.getWidgetsByLocations(
 ...
 */
function getCustomModesWithUniqueNames(
	builtinModes: readonly IChatMode[],
	customModes: readonly IChatMode[],
): Set<string> {
	...
	constructor(
		...
		this._register(
			chatModes.onDidChange(() => {
				...
				for (const mode of custom) {
					if (
						currentModeIds.has(mode.id) &&
						!this._modeActionDisposables.has(mode.id)
					) {
						this._registerModeAction(mode);
					}
				}
...
class HookSchemaAssociationContribution
	...
	private async _updateAssociations(): Promise<void> {
		...
		const folders = PromptsConfig.promptSourceFolders(
		...
		for (const folder of folders) {
			...
			if (
				...
				folder.source === PromptFileSource.ClaudePersonal
			) {
				continue;
			}
			...
			this._registrations.add(
				jsonContributionRegistry.registerSchemaAssociation(
					HOOK_SCHEMA_URI,
					glob,
				),
			);
...
class ToolReferenceNamesContribution
	...
	private _updateToolReferenceNames(): void {
		const tools = Array.from(
			this._languageModelToolsService.getAllToolsIncludingDisabled(),
		)
			.filter(
				(tool): tool is typeof tool & { toolReferenceName: string } =>
		...
		for (const tool of tools) {
			...
			toolReferenceNameEnumDescriptions.push(
				nls.localize(
...
agentPluginDiscoveryRegistry.register(
	new SyncDescriptor(ConfiguredAgentPluginDiscovery),
);
agentPluginDiscoveryRegistry.register(
	new SyncDescriptor(MarketplaceAgentPluginDiscovery),
);
agentPluginDiscoveryRegistry.register(
	new SyncDescriptor(ExtensionAgentPluginDiscovery),
);
agentPluginDiscoveryRegistry.register(
	new SyncDescriptor(CopilotCliAgentPluginDiscovery),
);
