...
import {
	IInlineChatWidgetConstructionOptions,
	InlineChatWidget,
} from "../../../inlineChat/browser/inlineChatWidget.js";
...
export class TerminalChatWidget extends Disposable {
	...
	public get inlineChatWidget(): TerminalInlineChatWidget {
	...
	private readonly _model: MutableDisposable<IChatModelReference> =
		this._register(new MutableDisposable());
	private readonly _sessionDisposables: MutableDisposable<IDisposable> =
		this._register(new MutableDisposable());
	...
	constructor(
		...
		this._register(
			Event.any(
				...
				Event.fromObservableLight(
					this._inlineChatWidget.chatWidget.input.selectedLanguageModel,
				),
				Event.debounce(
	...
	private _resetPlaceholder() {
		...
		this.inlineChatWidget.placeholder =
			...
			localize("askAboutCommands", "Ask about commands");
	}
	...
	private async _createSession(): Promise<void> {
		this._sessionCtor = createCancelablePromise<void>(async (token) => {
			if (!this._model.value) {
				const modelRef = this._chatService.startNewLocalSession(
					ChatAgentLocation.Terminal,
				);
		...
		this._sessionDisposables.value = toDisposable(() =>
	...
	async viewInChat(): Promise<void> {
		...
		for (const item of currentRequest.response.response.value) {
			if (item.kind === "textEditGroup") {
				...
				message.push(item as IChatProgress);
			}
		}
...
class TerminalInlineChatWidget extends InlineChatWidget {
	...
	selectAll() {
		this.chatWidget.inputEditor.setSelection(
