...
class LanguageModelResponse {
	...
	constructor() {
		...
		const [stream1, stream2] = AsyncIterableProducer.tee(
	...
	handleResponsePart(parts: IChatResponsePart | IChatResponsePart[]): void {
		...
		for (const part of Iterable.wrap(parts)) {
			...
			if (part.type === "text") {
				...
				out = new extHostTypes.LanguageModelThinkingPart(
					part.value,
					part.id,
					part.metadata,
				);
			} else if (part.type === "data") {
				out = new extHostTypes.LanguageModelDataPart(
					part.data.buffer,
					part.mimeType,
					part.audience,
				);
				...
				// step and runError types are not LM response parts, skip them
				continue;
			}
...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
	...
	readonly onDidChangeModelProxyAvailability =
		this._onDidChangeModelProxyAvailability.event;

	private readonly _languageModelProviders = new Map<
		string,
		LanguageModelProviderData
	>();
	// TODO @lramos15 - Remove the need for both info and metadata as it's a lot of redundancy. Should just need one
	private readonly _localModels = new Map<
		string,
		{
	...
	private _languageModelProxyProvider:
		| vscode.LanguageModelProxyProvider
		| undefined;
	...
	private getVendorFromModelIdentifier(
		...
		return firstSlash === -1
			? undefined
			: modelIdentifier.substring(0, firstSlash);
	}
	...
	async $provideLanguageModelChatInfo(
		...
		const modelInformation: vscode.LanguageModelChatInformation[] =
			(await data.provider.provideLanguageModelChatInformation(
				{ silent: options.silent, configuration: options.configuration },
				token,
			)) ?? [];
		const modelMetadataAndIdentifier: ILanguageModelChatMetadataAndIdentifier[] =
			modelInformation.map((m): ILanguageModelChatMetadataAndIdentifier => {
				let auth;
				if (
					m.requiresAuthorization &&
					isProposedApiEnabled(data.extension, "chatProvider")
				) {
					auth = {
						providerLabel: data.extension.displayName || data.extension.name,
						accountLabel:
							typeof m.requiresAuthorization === "object"
								? m.requiresAuthorization.label
								: undefined,
					};
				}
				...
				if (isProposedApiEnabled(data.extension, "chatProvider")) {
					if (m.isDefault === true) {
						...
						for (const key of Object.keys(m.isDefault)) {
							...
							isDefaultForLocation[typeConvert.ChatLocation.from(enumKey)] =
								m.isDefault[enumKey];
						}
					}
				}
				...
				return {
					metadata: {
						...
						configurationSchema: m.configurationSchema as
							| IJSONSchema
							| undefined,
	...
	async $startChatRequest(
		...
		if (!data) {
			throw new Error(
				`Language model provider for '${knownModel.metadata.id}' not found.`,
			);
		}
		...
		const sendNow = () => {
			if (queue.length > 0) {
				this._proxy.$reportResponsePart(
					requestId,
					new SerializableObjectWithBuffers(queue),
				);
				queue.length = 0;
			}
		};
		...
		try {
			value = data.provider.provideLanguageModelChatResponse(
				...
				// todo@connor4312: move `core` -> `undefined` after 1.111 Insiders is out
				{
					...
					toolMode:
						options.toolMode ?? extHostTypes.LanguageModelChatToolMode.Auto,
				},
				progress,
				token,
			);
	...
	$provideTokenLength(
		...
		return Promise.resolve(
			data.provider.provideTokenCount(knownModel.info, value, token),
		);
	}
	...
	async getDefaultLanguageModel(
		...
		for (const [modelIdentifier, modelData] of this._localModels) {
			if (
				...
				modelData.metadata.vendor === "copilot"
			) {
	...
	async selectLanguageModels(
		...
		const modelPromises = models.map((identifier) =>
			this.getLanguageModelByIdentifier(extension, identifier),
		);
	...
	async $acceptResponsePart(
		requestId: number,
		chunk: SerializableObjectWithBuffers<
	...
	private async _computeTokenLength(
		...
		return (
			this._languageModelProviders
				.get(data.metadata.vendor)
	...
	async getModelProxy(
		extension: IExtensionDescription,
	): Promise<vscode.LanguageModelProxy> {
		checkProposedApiEnabled(extension, "languageModelProxy");

		if (!this._languageModelProxyProvider) {
			this._logService.trace(
				"[LanguageModelProxy] No LanguageModelProxyProvider registered",
			);
			throw new Error("No language model proxy provider is registered.");
		}

		const requestingExtensionId = ExtensionIdentifier.toKey(
			extension.identifier,
		);
		try {
			const result = await Promise.resolve(
				this._languageModelProxyProvider.provideModelProxy(
	...
	registerLanguageModelProxyProvider(
		extension: IExtensionDescription,
		provider: vscode.LanguageModelProxyProvider,
	): vscode.Disposable {
		checkProposedApiEnabled(extension, "chatParticipantPrivate");
