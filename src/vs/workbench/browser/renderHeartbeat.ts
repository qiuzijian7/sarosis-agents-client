 ...
 *
 * ## 与既有诊断的分工（避免重复建设 —— 本仓还有两套，且都保留）
 *   · `[WsSwitchDiag]`：看门狗，**事件驱动** —— 某个被观测阶段跑慢时报「本窗口主线程被阻塞 N ms」。
 *     缺点：**没有脉搏**，它不说话就无法区分"没事"与"整段被冻住"。
 *   · `[StreamPerf]`：流式 delta 批次耗时（agent 面板局部）。
 *   ⇒ 两者的共同缺口是**连续性**：没人能回答"14:03:16 起窗口就没再动过"。
 *     本模块就补这一条：**不依赖任何业务事件**，固定间隔打点；打点消失 = 线程被占住。
 *   ★ 2026-09-22（2026-09-23 复核 ✓）：原先这里还列了第三套 `[FocusTrace]`
 *     （**焦点事件驱动** —— 量化"点一下到能绘制"的延迟 ✓）。该诊断已按用户要求
 *     **整体移除** ✓（曾被并行会话回退过一次 ✗ ⇒ 已重新移除 ✓），注释同步更新 ✓。
 *
 * ## 核心指标
