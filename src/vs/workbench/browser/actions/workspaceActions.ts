...
class OpenWorkspaceConfigFileAction extends Action2 {
	...
	constructor() {
		super({
			...
			category: workspacesCategory,
			f1: true,
			precondition: ContextKeyExpr.and(WorkbenchStateContext.isEqualTo('workspace'))
		});
	}
...
export class AddRootFolderAction extends Action2 {
	...
	constructor() {
		super({
			...
			category: workspacesCategory,
			f1: true,
			precondition: ContextKeyExpr.and(ContextKeyExpr.or(EnterMultiRootWorkspaceSupportContext, WorkbenchStateContext.isEqualTo('workspace')))
		});
	}
...
MenuRegistry.appendMenuItem(MenuId.MenubarFileMenu, {
	...
	when: ContextKeyExpr.and(ContextKeyExpr.or(EnterMultiRootWorkspaceSupportContext, WorkbenchStateContext.isEqualTo('workspace'))),
	order: 1
});
