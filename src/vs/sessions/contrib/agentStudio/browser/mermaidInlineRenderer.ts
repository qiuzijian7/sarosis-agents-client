...
export const IMermaidInlineRenderer = createDecorator<IMermaidInlineRenderer>('mermaidInlineRenderer');

export interface IMermaidInlineRenderer {
	readonly _serviceBrand: undefined;

	/**
	 * Renders Mermaid source markup to an SVG string.
...
/**
 * Renders Mermaid diagrams to SVG strings using a hidden off-screen webview
 * hosting the `index-render.js` bundle from the mermaid-chat-features
 * extension. Lives entirely in the renderer process — no extension host
 * round-trip, no visible editor tab.
 *
 * The webview is created lazily on first render request and kept alive for
 * subsequent requests. If it fails to initialize, requests reject and the
 * next request retries from scratch.
 */
export class MermaidInlineRenderer extends Disposable implements IMermaidInlineRenderer {
	...
	async renderToSvg(markup: string, theme?: 'dark' | 'default'): Promise<string> {
		if (!markup || !markup.trim()) {
			throw new Error('Mermaid 源码为空，无法渲染');
		}
		...
		if (!webview) {
			throw new Error('Mermaid 渲染 webview 不可用');
		}
		...
		const timer = setTimeout(() => {
			...
			this._logService.error('[MermaidInlineRenderer] render did not complete within ' + RENDER_TIMEOUT_MS + 'ms for request ' + requestId);
	...
	private async _createWebview(): Promise<void> {
		...
		// Off-screen container: mermaid needs a live DOM to measure text.
		...
		container.setAttribute('data-mermaid-inline-renderer', 'true');
		mainWindow.document.body.appendChild(container);
		this._container = container;

		const webview = this._webviewService.createWebviewElement({
			title: 'Mermaid Renderer (hidden)',
			options: {
...
<body>
	<script nonce="${nonce}">${safeJs}</script>
		...
		const readyPromise = new Promise<void>((resolve, reject) => {
			const timer = setTimeout(() => {
				listener.dispose();
				this._logService.error('[MermaidInlineRenderer] hidden webview did not signal ready within ' + READY_TIMEOUT_MS + 'ms');
				reject(new Error('Mermaid 渲染 webview 初始化超时'));
			}, READY_TIMEOUT_MS);
			...
			const listener = webview.onMessage((e) => {
				...
				if (msg.type === 'rendered' && typeof msg.requestId === 'string') {
					...
					if (msg.error || !msg.svg) {
						this._logService.error('[MermaidInlineRenderer] render returned no SVG: ' + JSON.stringify(msg.diag ?? {}) + ' error=' + (msg.error ?? 'empty'));
						pending.reject(new Error(msg.error || 'Mermaid 渲染返回空结果'));
					} else {
						this._logService.info('[MermaidInlineRenderer] rendered SVG ok: ' + JSON.stringify(msg.diag ?? {}));
		...
		await readyPromise;
		this._logService.info('[MermaidInlineRenderer] hidden render webview ready');
	}
	...
	private _teardown(): void {
		...
		for (const [, pending] of this._pending) {
			clearTimeout(pending.timer);
			pending.reject(new Error('Mermaid 渲染 webview 已销毁'));
		}
		this._pending.clear();
	}
