...
export interface IExecutableTool {
	readonly handler: (args: Record<string, unknown>, signal?: AbortSignal, agentId?: string) => Promise<ToolHandlerResult>;
...
export async function executeToolImpl(
	...
	try {
		const raw = await t.handler(toolCall.arguments ?? {}, signal, agentId);
