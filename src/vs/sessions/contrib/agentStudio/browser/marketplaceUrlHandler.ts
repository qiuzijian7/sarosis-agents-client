 ...
 *     → Writes MCP config directly to ~/.vssaros/mcp/{slug}/config.json and
 *       registers in ~/.vssaros/mcp.json (no zip download needed).
 *
 * Flow:
...
export class MarketplaceUrlHandler extends Disposable implements IURLHandler {
	 ...
	 * `~/.vssaros/mcp/{slug}/config.json` and register in `~/.vssaros/mcp.json`.
	 *
	...
	/**
	 * Write MCP config to `~/.vssaros/mcp/{slug}/config.json` and register
	 * in `~/.vssaros/mcp.json`. Mirrors the logic in mcpInstaller.ts.
	 */
	private async _writeMcpConfig(slug: string, name: string, config: McpConfig): Promise<{ configPath: string }> {
		...
		// 1. Create directory ~/.vssaros/mcp/{slug}/
		...
		// 3. Read existing ~/.vssaros/mcp.json (or create empty)
