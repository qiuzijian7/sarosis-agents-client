...
import { DATA_FILE_EMPLOYEES, DATA_FILE_WORKSPACES, DATA_FILE_SESSIONS, AGENT_STUDIO_DATA_PATH_SETTING } from '../common/constants.js';
...
export class AgentStudioService extends Disposable implements IAgentStudioService {
	...
	private async _readJsonFile<T>(filename: string): Promise<T[]> {
		try {
			const uri = URI.joinPath(this._getDataUri(), filename);
			const content = await this.fileService.readFile(uri);
			return JSON.parse(content.value.toString()) as T[];
		} catch (err) {
			this.logService.debug(`[AgentStudio] File not found or empty: ${filename}`);
			return [];
		}
	}
	...
	async getEmployees(workspaceId?: string): Promise<Employee[]> {
		const employees = await this._readJsonFile<Employee>(DATA_FILE_EMPLOYEES);
		if (workspaceId) {
			return employees.filter(e => e.workspaceId === workspaceId);
		}
		return employees;
	}
	...
	async createEmployee(data: Partial<Employee>): Promise<Employee> {
		...
		await this._writeJsonFile(DATA_FILE_EMPLOYEES, employees);
	...
	async deleteEmployee(id: string): Promise<void> {
		...
		await this._writeJsonFile(DATA_FILE_EMPLOYEES, filtered);
		this._onDidChangeEmployees.fire();
	}
	...
	async getWorkspaces(): Promise<Workspace[]> {
		return this._readJsonFile<Workspace>(DATA_FILE_WORKSPACES);
	}

	async getWorkspace(id: string): Promise<Workspace | undefined> {
		const workspaces = await this._readJsonFile<Workspace>(DATA_FILE_WORKSPACES);
	...
	async createWorkspace(data: Partial<Workspace>): Promise<Workspace> {
		...
		await this._writeJsonFile(DATA_FILE_WORKSPACES, workspaces);
	...
	async deleteWorkspace(id: string): Promise<void> {
		...
		await this._writeJsonFile(DATA_FILE_WORKSPACES, filtered);
		this._onDidChangeWorkspace.fire(id);
	}
	...
	async getSessions(): Promise<AgentStudioSession[]> {
		return this._readJsonFile<AgentStudioSession>(DATA_FILE_SESSIONS);
	}

	async getSession(id: string): Promise<AgentStudioSession | undefined> {
		const sessions = await this._readJsonFile<AgentStudioSession>(DATA_FILE_SESSIONS);
	...
	async createSession(data: Partial<AgentStudioSession>): Promise<AgentStudioSession> {
		...
		await this._writeJsonFile(DATA_FILE_SESSIONS, sessions);
	...
	async deleteSession(id: string): Promise<void> {
		...
		await this._writeJsonFile(DATA_FILE_SESSIONS, filtered);
		this._onDidChangeSessions.fire();
	}
}
