...
export function OrchestrationPlanDialog({ onClose }: OrchestrationPlanDialogProps): React.ReactElement {
	...
	// Approve plan — only PM can do this
	const handleApprove = useCallback(async () => {
		if (!activePlan || !hasPM) { return; }
		await approvePlan(activePlan.id);
	}, [activePlan, hasPM, approvePlan]);
	...
	// Task action — only PM can dispatch
				...
				{!activePlan && (
					<>
						...
						<div className="orch-role-status">
							...
							<div className={`orch-role-badge ${hasPM ? 'ok' : 'missing'}`}>
								{hasPM ? '✅' : '⚠️'} PM: {hasPM ? pm!.name : '未创建 — 请先创建一个 agentType=pm 的 Agent（仅限 1 个）'}
							</div>
						</div>
							...
							{isPendingApproval && (
								<>
									...
									<button
										...
										disabled={isLoading || !hasPM}
										title={!hasPM ? '需要 PM 才能调度执行' : 'PM 批准并开始调度'}
									>
										{isLoading ? '执行中...' : !hasPM ? '⚠️ 需要 PM 才能执行' : `✅ PM(${pm!.name}) 批准调度`}
									</button>
								</>
							)}
							{!isPendingApproval && (
