...
suite('detectSimilarCodeIncremental vs detectSimilarCode', () => {
	...
	test('full detection pairs identical signatures; incremental with an unrelated new node adds nothing', () => {
		...
		// 增量只查 n3（与谁都不同）→ 0 边
		const inc3 = detectSimilarCodeIncremental(new Set([n3.id]), all, store, 0.7);
		assert.strictEqual(inc3.length, 0, 'an unrelated new node must not produce any edge');
	});

	test('incremental with a new node identical to an existing one recovers the same edge', () => {
		const store = new CodebaseGraphStore();
		const n1 = addFn(store, 'fnA', sig(1000));
		...
		const inc2 = detectSimilarCodeIncremental(new Set([n2.id]), all, store, 0.7);
		assert.deepStrictEqual(sortedPairs(inc2), ['fnA<->fnB']);
	});

	test('incremental result is always a subset of full result and every edge touches a new node', () => {
		const store = new CodebaseGraphStore();
		...
		const incC = detectSimilarCodeIncremental(new Set([nC.id]), all, store, 0.7);
		assert.strictEqual(incC.length, 0);

		// 把 nB 当新节点 → 只应得到 fnA<->fnB，绝不能把 fnD<->fnE（两个都是旧节点）带出来
		const incB = detectSimilarCodeIncremental(new Set([nB.id]), all, store, 0.7);
		...
		const incBE = detectSimilarCodeIncremental(new Set([nB.id, nE.id]), all, store, 0.7);
		assert.deepStrictEqual(sortedPairs(incBE), ['fnA<->fnB', 'fnD<->fnE']);
	});

	test('new nodes that are clones of EACH OTHER are still detected', () => {
		const store = new CodebaseGraphStore();
		...
		const inc = detectSimilarCodeIncremental(new Set([nX.id, nY.id]), all, store, 0.7);
		assert.deepStrictEqual(sortedPairs(inc), ['fnX<->fnY'], 'new-vs-new clones must be detected');
	});

	test('empty or no-function new set short-circuits without throwing', () => {
		const store = new CodebaseGraphStore();
		addFn(store, 'fnA', sig(1000));
		addFn(store, 'fnB', sig(1000));
		const all = store.getAllNodes();

		assert.strictEqual(detectSimilarCodeIncremental(new Set<number>(), all, store, 0.7).length, 0);
		...
		assert.strictEqual(detectSimilarCodeIncremental(new Set([fileNode.id]), all, store, 0.7).length, 0);
	});
