 ...
 *
 * Verifies that:
 * 1. Button states (install/uninstall/installing) render correctly
 * 2. Install/uninstall actions emit proper events
 * 3. IntegrationView receives events and refreshes
 */
...
class MockEventBridgeService implements Partial<IEventBridgeService> {
	private _emitted: IEmittedEvent[] = [];
	readonly _listeners = new Map<string, ((event: any) => void)[]>();

	get emittedEvents(): IEmittedEvent[] { return this._emitted; }
...
interface MockMcpServer extends IMcpServer {
	_name: string;
	_connState: McpConnectionState;
	_cacheState: McpServerCacheState;
	_tools: any[];
}

function createMockServer(name: string, running: boolean): MockMcpServer {
	const connState = observableValue<McpConnectionState>('conn', {
		state: running ? McpConnectionState.Kind.Running : McpConnectionState.Kind.Disconnected
	});
	const cacheState = observableValue<McpServerCacheState>('cache', running ? McpServerCacheState.Live : McpServerCacheState.NotStarted);
	const tools = observableValue<readonly any[]>('tools', []);
...
class MockMcpManagementService implements Partial<IWorkbenchMcpManagementService> {
	private _installed: { name: string; id: string }[] = [];

	getInstalled(): Promise<any[]> {
		return Promise.resolve(this._installed.map(s => ({ name: s.name, id: s.id })));
	}

	async install(server: IInstallableMcpServer): Promise<any> {
	...
	async uninstall(server: any): Promise<void> {
		this._installed = this._installed.filter(s => s.name !== server.name);
	}

	// Test helpers
	_isInstalled(name: string): boolean {
 ...
 */
function getButtonState(installing: boolean, installed: boolean): { text: string; disabled: boolean; action: string } {
	if (installing) {
...
suite('MCP Detail Editor Pane — Button State Sync', () => {
	...
	let mockEventBridge: MockEventBridgeService;
	let mockMcpManagement: MockMcpManagementService;
	...
	test('安装成功后 emit mcp:servers-changed { action: "add" }', async () => {
		...
		await mockMcpManagement.install({ name: serverName, config: { type: 1, command: 'npx' } as any });
	...
	test('IntegrationView 收到 "add" 事件后应启用 MCP', () => {
		const sanitize = (s: string) => s.replace(/[^A-Za-z0-9_]/g, '_');
		const mcpDisabledIds = new Set<string>(['test_mcp']); // pre-disabled
	...
	test('IntegrationView 收到 "remove" 事件后应清理 startingMcpIds', () => {
		const sanitize = (s: string) => s.replace(/[^A-Za-z0-9_]/g, '_');
		const startingMcpIds = new Set<string>(['removing_mcp']);
		let reloadCalled = false;
	...
	test('完整安装→同步→卸载→同步周期', async () => {
		...
		const sanitize = (s: string) => s.replace(/[^A-Za-z0-9_]/g, '_');
		const mcpDisabledIds = new Set<string>();
	...
	test('白名单中的 MCP 被禁用时，auto-start 应清除过时禁用状态', () => {
		...
		const mcpDisabledIds = new Set<string>(['tapd']); // tapd is disabled but in whitelist

		// Simulate auto-start loop logic
		const srv = { id: 'tapd', name: 'tapd', status: 'disconnected' as const };
	...
	test('不在白名单中的 MCP 被禁用时，不清除禁用状态', () => {
		const sarosServerNames = new Set(['codebase-memory-mcp', 'tapd'].map(n => n.toLowerCase()));
		const mcpDisabledIds = new Set<string>(['old-removed-mcp']);

		const srv = { id: 'old-removed-mcp', name: 'old-removed-mcp', status: 'disconnected' as const };
		let enabled = !mcpDisabledIds.has(srv.id);
