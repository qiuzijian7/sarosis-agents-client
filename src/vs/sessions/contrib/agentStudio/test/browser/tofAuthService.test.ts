...
import { ITofUser, TofAuthError } from '../../common/tofAuth.js';
...
suite('Agent Studio - TOF Auth Service', () => {
	 ...
	 */
	function createMockConfigService(config: Record<string, unknown>): any {
		return {
	 ...
	 */
	function createMockOpenerService(): any {
		return {
			_serviceBrand: undefined,
			open: async () => true,
	...
	function createMockNotificationService(): any {
		...
		return {
			_serviceBrand: undefined,
			info: (msg: string) => notifications.push(msg),
			warn: (msg: string) => notifications.push(msg),
			error: (msg: string) => notifications.push(msg),
	...
	test('Mock 日志服务 — 记录日志', () => {
		...
		assert.ok(logService.getLogs().some((l: string) => l.includes('test message')));
	});
