/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the subAgentReturnFormat tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-subAgentReturnFormat-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './subAgentReturnFormat.test.ts');
...
const out = path.join(os.tmpdir(), `subagent-return-format-test-${Date.now()}.cjs`);
