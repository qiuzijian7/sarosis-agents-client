/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the toolCallParallelPolicy tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-toolCallParallelPolicy-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './toolCallParallelPolicy.test.ts');
...
const out = path.join(os.tmpdir(), `tool-parallel-policy-test-${Date.now()}.cjs`);
