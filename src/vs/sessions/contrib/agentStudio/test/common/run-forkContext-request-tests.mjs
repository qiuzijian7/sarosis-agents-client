/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the Fork 前缀缓存 request-construction-end tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-forkContext-request-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './forkContext-request.test.ts');
...
const out = path.join(os.tmpdir(), `fork-request-test-${Date.now()}.cjs`);
