...
const entry = path.join(__dirname, 'deliveryQueue.test.ts');
const outfile = path.join(os.tmpdir(), 'deliveryQueue.test.js');

const tsResolvePlugin = {
	name: 'ts-resolve',
	setup(b) {
		b.onResolve({ filter: /\.js$/ }, args => {
			// 只重写本仓库的 .ts 源（ESM 里 import 写 .js 实际是 .ts）；
			// node_modules 里的是真 .js，重写会指向不存在的 .ts 文件。
			if (path.resolve(args.resolveDir, args.path).includes('node_modules')) { return null; }
...
mocha.run(failures => {
	...
	else { console.log('DeliveryQueue tests PASSED'); }
});
