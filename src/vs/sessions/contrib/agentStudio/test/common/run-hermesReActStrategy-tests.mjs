/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the hermesReActStrategy tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-hermesReActStrategy-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './hermesReActStrategy.test.ts');
...
const out = path.join(os.tmpdir(), `hermes-react-strategy-test-${Date.now()}.cjs`);
