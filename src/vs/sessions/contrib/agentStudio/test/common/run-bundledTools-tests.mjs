/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the bundled-tools tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-bundledTools-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './bundledTools.test.ts');
...
const out = path.join(os.tmpdir(), `bundled-tools-test-${Date.now()}.cjs`);
