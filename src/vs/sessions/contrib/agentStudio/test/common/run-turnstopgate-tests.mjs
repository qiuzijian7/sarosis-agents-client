/*---------------------------------------------------------------------------------------------
 *  Bundler + runner for the turnStopGate tests (mirror of the gitRepoDiscovery test runner).
 *
 ...
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-turnstopgate-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, 'turnStopGate.test.ts');
...
const out = path.join(os.tmpdir(), `agentstudio-turnstopgate-test-${Date.now()}.mjs`);
