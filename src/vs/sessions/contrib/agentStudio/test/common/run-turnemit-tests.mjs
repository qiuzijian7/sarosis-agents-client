/*---------------------------------------------------------------------------------------------
 *  Bundler + runner for the turnEmit tests (mirror of the turnHookBus test runner).
 *
 ...
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-turnemit-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, 'turnEmit.test.ts');
...
const out = path.join(os.tmpdir(), `agentstudio-turnemit-test-${Date.now()}.mjs`);
