/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for subAgentCardReducer pure-function tests.
 *  esbuild bundles the .test.ts (resolving .js→.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-subAgentCardReducer-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './subAgentCardReducer.test.ts');
...
const out = path.join(os.tmpdir(), `card-reducer-test-${Date.now()}.cjs`);
