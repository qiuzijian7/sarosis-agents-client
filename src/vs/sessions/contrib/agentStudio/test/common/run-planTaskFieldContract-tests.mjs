/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the plan task field-contract tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-planTaskFieldContract-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './planTaskFieldContract.test.ts');
...
const out = path.join(os.tmpdir(), `plan-task-field-contract-test-${Date.now()}.cjs`);
