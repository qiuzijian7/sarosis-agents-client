/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the mimoStrategy tests.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-mimoStrategy-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './mimoStrategy.test.ts');
...
const out = path.join(os.tmpdir(), `mimo-strategy-test-${Date.now()}.cjs`);
