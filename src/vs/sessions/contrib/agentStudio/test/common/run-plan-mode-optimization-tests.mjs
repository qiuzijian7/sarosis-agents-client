/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the plan-mode optimization tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-plan-mode-optimization-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './plan-mode-optimization.test.ts');
...
const out = path.join(os.tmpdir(), `plan-mode-opt-test-${Date.now()}.cjs`);
