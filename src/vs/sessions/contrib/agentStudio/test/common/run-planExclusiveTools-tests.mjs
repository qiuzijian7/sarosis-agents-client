/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the plan-exclusive tools gate tests.
...
const entry = path.resolve(import.meta.dirname, './planExclusiveTools.test.ts');
...
const out = path.join(os.tmpdir(), `plan-exclusive-tools-test-${Date.now()}.cjs`);
