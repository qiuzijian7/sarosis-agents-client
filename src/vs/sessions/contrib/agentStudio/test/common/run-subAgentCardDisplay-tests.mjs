/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for subAgentCardDisplay pure-function tests.
...
const entry = path.resolve(import.meta.dirname, './subAgentCardDisplay.test.ts');
...
const out = path.join(os.tmpdir(), `card-display-test-${Date.now()}.cjs`);
