/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the historyCompaction tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-historyCompaction-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './historyCompaction.test.ts');
...
const out = path.join(os.tmpdir(), `history-compaction-test-${Date.now()}.cjs`);
