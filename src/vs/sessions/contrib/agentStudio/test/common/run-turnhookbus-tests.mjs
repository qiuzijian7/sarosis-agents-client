/*---------------------------------------------------------------------------------------------
 *  Bundler + runner for the turnHookBus tests (mirror of the turnStopGate test runner).
 *
 ...
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-turnhookbus-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, 'turnHookBus.test.ts');
...
const out = path.join(os.tmpdir(), `agentstudio-turnhookbus-test-${Date.now()}.mjs`);
