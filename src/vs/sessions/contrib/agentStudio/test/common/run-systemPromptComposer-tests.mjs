/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the systemPromptComposer tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/common/run-systemPromptComposer-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, './systemPromptComposer.test.ts');
...
const out = path.join(os.tmpdir(), `system-prompt-composer-test-${Date.now()}.cjs`);
