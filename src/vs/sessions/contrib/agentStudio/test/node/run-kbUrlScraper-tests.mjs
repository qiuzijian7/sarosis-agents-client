/*---------------------------------------------------------------------------------------------
 *  Bundler + mocha runner for the KB URL scraper pure-function tests.
 *  esbuild bundles the .test.ts (resolving .js->.ts), then mocha (tdd ui) runs it.
 *
 *  Usage (from the repo root):
 *      node src/vs/sessions/contrib/agentStudio/test/node/run-kbUrlScraper-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, '../../browser/views/knowledgeBase/test/node/kbUrlScraper.test.ts');
...
const out = path.join(os.tmpdir(), `kb-url-scraper-test-${Date.now()}.cjs`);
