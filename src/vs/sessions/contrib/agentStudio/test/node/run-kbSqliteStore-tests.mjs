/*---------------------------------------------------------------------------------------------
 *  KB SQLite 存储单元测试（esbuild + mocha，纯 Node.js 环境）。
 *
 *  用法：node run-kbSqliteStore-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(__dirname, 'kbSqliteStore.test.ts');
...
const out = path.join(projectRoot, 'tmp', `kbSqliteStore-test-${Date.now()}.cjs`);
...
await esbuild.build({
	...
	external: ['better-sqlite3', '@vscode/sqlite3', '../../node/kbSqliteStore.js'],
...
const mocha = new Mocha({ ui: 'bdd', timeout: 10000, reporter: 'spec' });
mocha.addFile(out);
mocha.run(failures => {
