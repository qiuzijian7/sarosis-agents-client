/*---------------------------------------------------------------------------------------------
 *  Bundler + runner for the gitRepoDiscovery tests (mirror of the engine test runner).
 *
 ...
 *      node src/vs/sessions/contrib/agentStudio/common/__tests__/run-gitdiscovery-tests.mjs
 *--------------------------------------------------------------------------------------------*/
...
const entry = path.resolve(import.meta.dirname, 'gitRepoDiscovery.test.ts');
...
const out = path.join(os.tmpdir(), `kb-gitdiscovery-test-${Date.now()}.mjs`);
