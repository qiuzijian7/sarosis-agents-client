...
activation: manual
match: [humanizer, writing, editing, humanize, anti-ai-slop, voice, prose, text]
category: creative
version: 2.5.1
---
...
2. **File** — user points at a file. Use `file_read` to load it, then `patch` or `file_write` to apply edits. For markdown docs in a repo, a targeted `patch` per section is cleaner than rewriting the whole file.
...
1. Read the input text carefully (use `file_read` if it's a file).
...
10. If the text came from a file, apply the edit with `patch` (targeted) or `file_write` (full rewrite) and show the user what changed.
