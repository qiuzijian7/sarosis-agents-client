...
- **Built-in skills**: Discovers bundled `SKILL.md` files from `vs/sessions/contrib/agentStudio/browser/.agents/skills/{name}/` and surfaces them with `PromptsStorage.builtin` storage type
...
All built-in customizations bundled with the Sessions app are skills, living in `src/vs/sessions/contrib/agentStudio/browser/.agents/skills/{name}/SKILL.md`. They are:

- Discovered at runtime via `FileAccess.asFileUri('vs/sessions/contrib/agentStudio/browser/.agents/skills')`
