 ...
 *
 * 用法：
 *   node build/saros/verify-product-branding.mjs            # 仅检测，不一致则退出码 1
 *   node build/saros/verify-product-branding.mjs --fix      # 检测并自动修复
 *
 * 退出码：
 *   0  全部一致（或 --fix 成功修复）
...
function main() {
	...
	if (!FIX) {
		console.log('');
		log('!', '运行 `node build/saros/verify-product-branding.mjs --fix` 可自动修复');
		process.exit(1);
	}
