...
- `quality`  ← product.json 的 `quality`（本项目为 `sarosis`）
- `commit`   ← 客户端当前 commit
...
```json
"quality": "sarosis",
"updateUrl": "https://vssarosis-update.example.workers.dev"
```
...
```bash
npm i -g wrangler
cd build/sarosis/update-server
...
# 私有仓库
GH_REPO=owner/repo GH_TOKEN=ghp_xxx node build/sarosis/update-server/server.mjs
```
