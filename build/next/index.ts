...
const desktopResourcePatterns = [
	...
	'vs/sessions/contrib/agentStudio/browser/.agents/skills/**/SKILL.md', // new path (Hermes-Agent inspired)
];
...
async function transpileCapabilityPlugins(outDir: string): Promise<void> {
	...
	// Generate the capability-plugins manifest as a JS module for runtime auto-discovery.
	// We use a .js module (not .json) because VSCode's ESM loader does not support
	// JSON import assertions. The module exports a `plugins` array that
	// AgentCapabilityPluginContribution reads at startup to dynamically import()
	// each plugin without hardcoded paths.
	const manifestDir = path.join(REPO_ROOT, outDir, 'vs', 'extensions');
	await fs.promises.mkdir(manifestDir, { recursive: true });
	const manifestJs = [
		'// Auto-generated by build/next/index.ts — do not edit manually.',
		`export const plugins = ${JSON.stringify(DISCOVERED_CAPABILITY_PLUGINS, null, '\t')};`,
		'',
	].join('\n');
	await fs.promises.writeFile(path.join(manifestDir, 'capability-plugins.js'), manifestJs, 'utf-8');
	console.log(`[transpile] Generated capability-plugins.js (${DISCOVERED_CAPABILITY_PLUGINS.length} plugins)`);

	// Also generate extensions-registry.js — a lightweight list of extension IDs
	// that AgentCapabilityPluginContribution can use as a secondary discovery
	// mechanism when capability-plugins.js fails to load (e.g. partial rebuild).
	const allExtIds = DISCOVERED_CAPABILITY_PLUGINS.map(p => p.id);
	const registryJs = [
		'// Auto-generated by build/next/index.ts — do not edit manually.',
		`export const extensionIds = ${JSON.stringify(allExtIds)};`,
		'',
	].join('\n');
	await fs.promises.writeFile(path.join(manifestDir, 'extensions-registry.js'), registryJs, 'utf-8');
	console.log(`[transpile] Generated extensions-registry.js (${allExtIds.length} extension IDs)`);
}
