...
## Rate Limit Handling
- Anonymous access works without a key (lower rate limits); high-frequency anonymous use will be rate-limited.
- On rate limit with an auto_registered api_key in the response, the CLI prints `AUTO_REGISTERED_API_KEY: <key>`: present the key to the user for approval, then on approval save `ANYSEARCH_API_KEY=<key>` to `~/.vssaros/skills/.hub/anysearch.env` (writable user dir — the builtin skill dir is read-only) and retry the command.
- On anonymous quota exhausted without a key: inform the user that a key provides higher limits; suggest saving one to `~/.vssaros/skills/.hub/anysearch.env` or setting the ANYSEARCH_API_KEY environment variable.
