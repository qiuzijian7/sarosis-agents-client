...
def _load_env():
    ...
    1. Next to this CLI script (``scripts/.env``).
    2. The skill root (parent of ``scripts/``) — both of the above live inside the
       **builtin** skill directory, which is READ-ONLY at runtime, so they are only
       useful for development / packaging, never for runtime key persistence.
    3. Writable user locations (desktop runtime): ``~/.vssaros/skills/.hub/anysearch.env``
       and ``~/.vssaros/skills/anysearch/.env`` (user-installed anysearch skill).
       These are the correct places to persist an auto-registered or user-provided key,
       because the builtin skill dir cannot be written to.
    """
    ...
    # Writable user data dirs. Packaged app uses ~/.vssaros/, dev builds use
    # variants like ~/.vssaros-dev/ or ~/.vssaros-agents-dev/ — glob all of them.
    for data_dir in sorted(glob.glob(os.path.join(home, ".vssaros*"))):
        candidate_paths.append(os.path.join(data_dir, "skills", ".hub", "anysearch.env"))
        candidate_paths.append(os.path.join(data_dir, "skills", "anysearch", ".env"))
...
def _extract_auto_key(payload) -> str:
    """Find an auto-registered api_key anywhere in an error payload (dict/list, nested)."""
    if isinstance(payload, dict):
        for k, v in payload.items():
            if k == "api_key" and isinstance(v, str) and v.strip():
                return v.strip()
            found = _extract_auto_key(v)
            if found:
                return found
    elif isinstance(payload, list):
        for item in payload:
            found = _extract_auto_key(item)
            if found:
                return found
    elif isinstance(payload, str):
        # Some backends embed the key in a plain-text error message.
        try:
            return _extract_auto_key(json.loads(payload))
        except Exception:
            return ""
    return ""


def _report_auto_key(payload) -> None:
    """If the error payload carries an auto-registered api_key, surface it with a
    stable, greppable marker so the calling agent can run the SKILL.md
    approval → save → retry flow."""
    key = _extract_auto_key(payload)
    if not key:
        return
    print("=" * 60, file=sys.stderr)
    print(f"AUTO_REGISTERED_API_KEY: {key}", file=sys.stderr)
    print("RATE LIMIT: the service auto-registered an API key for you.", file=sys.stderr)
    print("Per SKILL.md: present the key to the user for approval, then save", file=sys.stderr)
    print("ANYSEARCH_API_KEY=<key> to ~/.vssaros/skills/.hub/anysearch.env", file=sys.stderr)
    print("(writable user dir; do NOT write into the read-only builtin skill dir),", file=sys.stderr)
...
def _call_api(tool_name: str, arguments: dict, api_key: str) -> str:
    ...
    if "error" in data:
        ...
        _report_auto_key(data.get("error"))
        sys.exit(1)
