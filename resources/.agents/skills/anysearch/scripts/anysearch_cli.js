...
function loadEnv() {
  ...
  // variants like ~/.vssaros-dev/ or ~/.vssaros-agents-dev/ — scan all of them.
  try {
    const home = require("os").homedir();
    for (const entry of fs.readdirSync(home)) {
      if (entry.startsWith(".vssaros")) {
        envPaths.push(path.join(home, entry, "skills", ".hub", "anysearch.env"));
        envPaths.push(path.join(home, entry, "skills", "anysearch", ".env"));
      }
    }
  } catch (_) { /* home dir unreadable — builtin .env lookup still applies */ }
...
function findAutoKey(payload) {
  // Find an auto-registered api_key anywhere in an error payload (object/array/string).
  if (payload && typeof payload === "object") {
    const values = Array.isArray(payload) ? payload : Object.entries(payload);
    for (const entry of values) {
      const [k, v] = Array.isArray(payload) ? [null, entry] : entry;
      if (k === "api_key" && typeof v === "string" && v.trim()) return v.trim();
      const found = findAutoKey(v);
      if (found) return found;
    }
  } else if (typeof payload === "string") {
    try { return findAutoKey(JSON.parse(payload)); } catch (_) { return ""; }
  }
  return "";
}

function reportAutoKey(payload) {
  // Surface an auto-registered api_key with a stable, greppable marker so the
  // calling agent can run the SKILL.md approval → save → retry flow.
  const key = findAutoKey(payload);
  if (!key) return;
  const bar = "=".repeat(60);
  console.error(bar);
  console.error(`AUTO_REGISTERED_API_KEY: ${key}`);
  console.error("RATE LIMIT: the service auto-registered an API key for you.");
  console.error("Per SKILL.md: present the key to the user for approval, then save");
  console.error("ANYSEARCH_API_KEY=<key> to ~/.vssaros/skills/.hub/anysearch.env");
  console.error("(writable user dir; do NOT write into the read-only builtin skill dir),");
