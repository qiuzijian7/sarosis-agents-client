...
description: VsSarosis（VS Code fork）一键发版——编译+bundle+打包用户级安装包 → 上传 DevCloud → 在工蜂 vssarosis_issue 创建 Release → 打 git tag。更新代理自动从 Release 解析下载链接+SHA256，已安装用户下次启动自动更新。
...
VsSarosis（`sarosis-agents-client`，路径：`G:\CustomWorkspaces\AIProjects\sarosis-agents-client`）端到端发版流程。
...
3. **Release 仓库**：`https://git.woa.com/zijianqiu/vssarosis_issue`（项目 ID: 1790708），不是 sarosis-agents-client
4. **更新代理**：DevCloud `http://zijianqiu-any1.devcloud.woa.com:3030`，自动从工蜂 Release 描述提取下载 URL + SHA256
5. **Git 远程**：`origin`=工蜂 sarosis-agents-client / `backup`=GitHub / `upstream`=microsoft/vscode（禁止 push）
6. **product.json**：`quality: "sarosis"`，`updateUrl: "http://zijianqiu-any1.devcloud.woa.com:3030"`
...
scp .build/win32-x64/user-setup/VsSarosisUserSetup.exe \
    root@zijianqiu-any1.devcloud.woa.com:/opt/vssarosis-update/downloads/
```
...
git push origin v0.1.X     # 工蜂 sarosis-agents-client
git push backup v0.1.X     # GitHub
```

### Stage 4：创建工蜂 Release（vssarosis_issue）

> ⚠️ **关键**：Release 创建在 `zijianqiu/vssarosis_issue` 仓库（ID: 1790708），不是 sarosis-agents-client。
...
响应示例：`{"url":"/uploads/xxxxxxxx/VsSarosisUserSetup.exe"}`，拼接为 `https://git.woa.com/zijianqiu/vssarosis_issue/uploads/xxxxxxxx/VsSarosisUserSetup.exe`

**步骤 4.2**：在 vssarosis_issue 创建 tag（如不存在）
...
```bash
# 测试更新 API（用旧 commit）
curl -s http://zijianqiu-any1.devcloud.woa.com:3030/api/update/win32-x64/sarosis/old-commit
...
- **Release 必须在 vssarosis_issue 仓库创建**（不是 sarosis-agents-client），需要先在 vssarosis_issue 建 tag
